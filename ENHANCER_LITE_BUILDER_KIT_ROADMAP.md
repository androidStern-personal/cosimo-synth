...
## September 9 — current recovery and execution

This is the complete Builder Kit roadmap, now restored to the maintained product source. Andrew authorized the recovery plan with "get it done" in task `01a08549-db5f-7dc1-ad7b-bb91acf0802b`. That task is the current integration and readiness coordinator. Current assignments and the single integration queue are at the top of `TODOS.txt`; the current customer contract and release locations are in [the launch handoff](ENHANCE_THAT_LAUNCH_HANDOFF.md). These current entries supersede historical owners, schedules and incomplete status claims below. The older roadmap text remains intact.
...
| Ticket | Original task name | September 9 reconciliation |
|---|---|---|
| BK-00 | Lock roadmap | Full plan recovered intact. Current scope is the superseding handoff plus later direct decisions. |
| BK-01 | Cmajor builder/OEM inquiry | Recorded Cmajor decision settled; retain the exact-artifact no-JIT boundary. No new legal review here. |
| BK-02 | JUCE builder/OEM inquiry | Recorded JUCE decision settled; customer-held license disclosure remains. Current novice acknowledgment issue is open. |
| BK-03 | Dependency and asset SBOM | L3 final dependency/notices inventory exists on the private product branch; not on source master. |
| BK-04 | Encode customer license and notices | Customer rights/license foundation landed; final product notices on L3 branch. Existing decisions retained. |
| BK-05 | Choose and check the public name | Enhance That selected and applied in private product/site. This audit did not establish a completed independent name-clearance check. |
| BK-06 | Choose Gate 0 path | Rights/framework path recorded settled. Do not revive the historical unresolved gate. |
| BK-07 | Reconcile repo task authority | Distinct launch handoff adopted to master; current tracking/navigation remains fragmented. |
| BK-10 | Freeze release feature contract | Private 0.1.3 composition exists; exact final beta freeze and human acceptance are not established. |
| BK-11 | Create sanitized export | Sanitized export foundation landed; current 146-file export tied to source 9928d599. |
| BK-12 | Isolate product state, host parameters, and tests | State isolation foundation landed; later automation/gesture/state-repair work is in private launch composition. |
| BK-13 | Create product identity system | Identity framework landed; current rename and migration metadata in launch composition. |
| BK-13A | Separate saved presets automatically by plugin identity | Merged preset isolation; additional derivative regression repair composed into L3. |
| BK-13B | Preserve human-readable plugin names in the DAW | Readable-name framework work merged and host evidence retained; new product presentation in L3. |
| BK-14 | Build Wavefold reference | Separate clean Wavefold reference a908b31f survives with DSP/native-build evidence; final accepted-candidate reference/tag and human acceptance remain distinct. |
| BK-15 | Customer repo history/package | Ordinary release mirror through 0.1.2; private 0.1.3 export/mirror exists; no public promotion established. |
| BK-20 | Finish the customer toolchain contract | Pinned tooling/CPM foundation landed. macOS15-capable compiler and current guest build proved in private launch work. |
| BK-21 | Implement doctor/setup | Setup/doctor implemented and exercised; current requested novice dependency/remediation/acknowledgment improvements not implemented. |
| BK-21A | Print a complete copy-paste command for JUCE acknowledgment | Full acknowledgment command implemented historically. Current outer-installer consent experience is the user-reported issue; do not mark it resolved. |
| BK-22 | Implement test/build/install | Build/install foundation landed. Current exported customer VST3/AU build evidence exists; human/end-to-end boundaries remain. |
| BK-22A | Investigate compiled-plugin Spacebar capture and shared host-keyboard regression | Investigation followed by authorized shared keyboard repair, merged c297eeed; retained real-host proof. |
| BK-22B | Replace only the intended plugin and retain install recovery | Identity-safe replacement and recovery merged; renamed-product and package recovery extensions in private L3. |
| BK-22C | Make the optional local browser UI entry point work | Optional real browser entry foundation merged; manifest-title correction composed into L3. |
| BK-23 | Implement update/recover | Update safeguards/docs landed. Real success/conflict/abort evidence uses earlier e33dd8da candidate, not final 9928d599. |
| BK-30 | Real AU production build | Actual AU built and included with VST3 in final signed private package; earlier AU-absent status is obsolete. |
| BK-31 | Vendor signing/notarization | Signed/notarized/stapled private package exists at f30f358f. No public-release implication. |
| BK-32 | Release qualification automation | Focused/package/customer evidence retained. Review-only manifest still qualification pending; no blanket completion claim. |
| BK-33 | Host and musical acceptance | AU/VST3 Ableton saved-state/audio evidence retained; subjective listening and full gesture/matrix acceptance not established. |
| BK-34 | Clean-machine matrix | macOS15 guest customer setup/native build recorded. Full clean downloaded-package/host matrix remains a distinct gap. |
| BK-40 | Setup and update prompts | Concise setup/update/support guidance exists; new guided-agent install request is open. Customer publishing deferred by later scope. |
| BK-40A | Provide and prove the approved first-use starting prompt | Unchanged included-plugin first request shipped and earlier customer proof retained; new novice entry flow must be reworked. |
| BK-41 | Documentation/onboarding HTML | Concise onboarding guidance retained; historical mandatory branded onboarding requirements superseded. Current success-page/agent experience open. |
| BK-42 | Support system | Support docs and beta materials exist. Final novice comprehension/support route acceptance not fully established. |
| BK-50 | Polar organization/compliance approval | Polar sandbox product and provider setup in use; production financial/publication boundaries remain separate. |
| BK-51 | Agent-operable commerce integration | Guest checkout, durable email/delivery/recovery fixes committed in website repository. Current new landing integration uncommitted; user saves deferred. |
| BK-52 | Landing page and Resend email flow | Public waitlist landing deployed; protected checkout landing deployed separately. Pre-purchase requirements and practical waitlist launch-email workflow remain open. |
| BK-60 | Demo production | Desktop/mobile loop assets and filmed materials exist. Original historical video format/presentation is not automatically current scope. |
| BK-61 | Press/outreach package | Recruitment/creative drafts exist; final press package and authorized outreach not established. |
| BK-70 | Beta protocol and recruitment | Beta operations materials committed locally. Last recorded recruitment was five people; live roster not reverified. |
| BK-71 | Run private beta | No completed actual seven-day frozen beta evidenced. |
| BK-72 | Beta repair and requalification | Post-beta repairs/requalification not complete before beta occurs. |
| BK-80 | Founding launch | Founding paid launch not complete. Public waitlist and private sandbox are different milestones. |
| BK-81 | Founding follow-up | Post-launch follow-up remains future work; not completed by drafting. |
| BK-82 | Price/scale decision | Future evidence-based price/scale decision; no automatic increase approved by this audit. |

### Later additions and scope changes

- BK-21B: one-command/short public entry delivered and merged through 7341f963. New destination and agent-guided setup request is a further change, not already implemented.
- BK-24A/B/D: build configuration/incremental generation/width-hack removal merged; BK-24C native qualification and BK-24E guidance cleanup merged through 1e57fd3f.
- BK-52A and later creative changes: retained media/design work is covered by L5/Woods/studio evidence. The old L5 packet hold does not cancel later directly authorized studio and landing work.
- September 6 guest-store correction: preserved in the approved implementation brief and live TODO; replaces old account/email-gate assumptions.
- September 9 BK-52/L4 requirement: disclose supported hardware, OS and build prerequisites before purchase; recorded in recovered roadmap, with live TODO pointer.
- September 9 install discussion: Documents/Builder Kit destination, agent-guided installation, dependency checks/remediation and visible JUCE acknowledgment remain requested/proposed work. Deep-link capabilities were not implemented.

### Remaining uncertainty

The original reference example a908b31f, older update experiment e33dd8da, final customer export 9928d599/1bf9e893, final free package f30f358f, and filmed derivatives are distinct. A pass on one is not a pass on all. Missing worktrees leave their former uncommitted contents unproven; this table does not erase that uncertainty.


---

## September 9, 2026 — roadmap recovery and new purchase requirement

The complete 1,117-line roadmap below was recovered from planning commit `46d709db074b030807adc048123f3e1f76fda9e2`; its temporary worktree no longer exists. This durable planning worktree preserves all of that text and the accompanying planning documents. The sibling consolidated handoff has been refreshed from its September 6 saved version; later execution status and direct Andrew decisions still supersede historical status claims. See [recovery evidence](BUILDER_KIT_ROADMAP_RECOVERY.md). Do not infer completion or revive superseded requirements from older sections.

- [ ] **BK-52 / L4 — Show system requirements before purchase.** Andrew requested September 9: clearly display Apple Silicon Mac and macOS 15 or newer on the Builder Kit product/landing page and purchase entry, matching the delivered installer. Explain that building/customization needs Apple Command Line Tools with guided setup, while Node/npm and CMake are supplied automatically. Distinguish Builder Kit development prerequisites from free-plugin playback requirements; supported OS/format claims must match the qualified release. Requirements must be readable near the purchase action on desktop and mobile, not first disclosed after payment or in the installer.

**Current execution handoff:** [Enhance That — consolidated launch handoff](ENHANCE_THAT_LAUNCH_HANDOFF.md), prepared September 5, 2026; Andrew has now authorized dispatch. It maps every BK ticket into six remaining work packets, records current evidence and defines the recruitment/beta/launch gates. Latest staffing: management-only Bob; visible top-level Create Task API workers, Astra/xhigh by default with Max/High discretion, standard speed and Full Access. Use it instead of older statuses, dated schedules or superseded requirements below. This document remains historical background; do not merge its older planning lineage wholesale into source master.

## September 5, 2026 — Enhance That decisions and parallel launch queue

Authority: Andrew's decisions in Woods thread `01a03885-0e68-7411-8b3f-f69ae41b6d89`. This section supersedes conflicting older naming, format/host, customer-publishing, refund and scheduling text below. It records planning decisions and queued work, not implementation completion or authorization to publish, spend, install, contact testers or send campaigns.

### Decisions now settled

- **Names:** free plugin **Enhance That**; paid product **Enhance That Builder Kit**. Name checks and final branding assets remain work; choosing a public name does not authorize changing stable plugin/parameter IDs or breaking saved sounds.
- **Platforms and formats:** macOS 15 and 26 on Apple Silicon remain in scope. VST3 remains required. Andrew subsequently removed Logic Pro as a blocker: if Logic is unavailable, qualify AU in a suitable free host if practical; otherwise defer AU and launch VST3-only. No paid DAW purchase is authorized. Stop the present Logic-access check; do not advertise Logic/AU compatibility without the corresponding proof.
- **Customer publishing:** built-in tooling for packaging/signing customers' modified plugins is deferred until after v1. Local build/install and broad customer rights remain. Signing/notarization of our own finished public plugin release is a separate retained release requirement.
- **Recruitment:** Andrew has five testers. Prepare a showable product, site and demo/press material so he can recruit the remainder before the formal beta. The target remains ten testers plus two backups; recruitment assets do not need to be the final launch film. Do not treat the incomplete roster as a prerequisite for preparing those assets.
- **Refunds:** generous case-by-case review, with no fixed 14-day cutoff or advertised unlimited guarantee. Approved public wording: "For refund requests, contact support with the reason for your request. We review each request individually." Andrew may approve a reasonable request regardless of purchase age; this does not authorize agents to issue refunds.
- **Support:** retain bounded email help for purchases, downloads and genuine setup problems, not custom development or bespoke merge support. Andrew delegated the minor response-time wording choice; publish no fixed response-time guarantee.
- **Planning and schedule:** retain required proof instead of treating the old dated calendar as current. Branding/creative decisions are queued work, not a reason to stop the rest of planning or independent engineering/commerce preparation. Paid advertising, affiliates, automated release/hosted-Mac CI and unrelated data research remain deferred or separate; this conversation does not reopen them.

### Queued BK-52A — launch creative concepts and recruitment-ready brief

- **Status:** queued for the launch handoff, not started. Product/creative owner: Woods; implementation owner not assigned. This is a scoped addition to BK-52, BK-60 and BK-61, not a rival engineering coordinator.
- **Input/dependencies:** the selected names, the existing free-plugin-to-paid-kit offer, current real product evidence and the decisions above. It does not wait for a finished AU, the final tester roster, or final branding approval to propose options.
- **Deliverable:** two or three coherent concepts, each combining a wordmark/visual direction (type and palette), landing-page composition and draft headline/product/offer copy, a real-workflow demo angle, and sample press-kit presentation. Show complete alternatives Andrew can react to; do not replace this work with a questionnaire about minor design settings.
- **Shared content contract:** distinguish the useful free plugin from the paid editable kit; retain the $29 first-50 founding offer and broad customer rights with the existing third-party disclosures. Use real product screenshots/audio when claiming actual behavior; clearly label mockups and unverified compatibility. Do not invent testimonials, an instant-build promise, customer distribution tooling or licensing inclusion.
- **First milestone after direction review:** a recruitment-ready package consisting of a presentable product page, real current screenshots, a short honest product/modification demonstration and a compact press sheet. Coordinate its parts through the existing BK-52 website, BK-14/BK-60 example/demo and BK-61 press tasks. Final polished launch video, full email campaign and publication are later deliverables, not prerequisites to recruiting the remaining testers.
- **Done when:** options and their draft copy/assets are saved in a reviewable handoff, the recommended direction and tradeoffs are clear, and the subsequent recruitment-package work has explicit deliverables and dependencies. Andrew's selection is required before finalizing that creative direction, not before unrelated work proceeds. A concept review alone does not complete the website, demo or press work.
- **Parallel work remains queued:** BK-52 product site/copy/email, BK-51 checkout/download/recovery, BK-60 real demo/audio, BK-61 press/creator outreach preparation, and BK-70 beta materials/recruitment support. These may be planned and prepared alongside product engineering, using provisional copy where needed. Public claims, sends, spending and release still require their own approval and verification; nothing starts solely because this entry exists.

## September 4, 2026 — approved Spacebar and plugin-name cleanup

Andrew delegated BK-22A (reproduce and repair shared Spacebar/host forwarding) and BK-13B (correct DAW display names without identity or naming regressions) to **Bob — Build reliability and AGENTS cleanup**, thread `01a06ded-e922-7b60-83a7-44332a0e3fee`. Bob owns reviewed implementation, integration and master push; this explicitly supersedes the old investigation-only/routine-repair holds for these items. Full scope: [BUILDER_KIT_HOST_CLEANUP.md](BUILDER_KIT_HOST_CLEANUP.md).

After both land, **Woods conducts the complete customer experience** using a fresh isolated candidate containing the exact fixes and matching tools, then reports results to Andrew. No old-release substitution, custom example wrapper, or long maintainer setup recipe. Candidate staging is separate from customer publication; no new public release or automatic CI/release infrastructure is authorized. Other roadmap holds remain unchanged.

## Deferred — automated, verified Builder Kit releases

- [ ] Explore a release pipeline that creates a candidate, verifies that exact candidate, and only then makes it available to customers; a failed gate must leave the previous release available.
- **Out of current scope**, explicitly deferred by Andrew on September 4, 2026. Not a launch blocker or unfinished part of BK-24. No implementation task, runner setup, spending, or automatic publishing is authorized.
- Prefer avoiding paid GitHub Mac compute. A VM on Andrew's existing Mac is one option, not a selected architecture. Decide triggering, required verification, resource isolation, credentials, and manual versus automatic publication when this work is reopened.
...
Integrated source: `origin/master@b40b52aa3236774dc3d6cd4bdbc1fe804d39c26c`

Dedicated reconciliation branch: `codex/builder-kit-roadmap-cleanup-20260903`; this older planning lineage is not merged into source master.

Historical integration baseline at roadmap creation: `origin/master@e3b832ebe8010a40a40e86780bd549467ed744d1`

Dependency-migration authority: `BUILDER_KIT_CPM_DEPENDENCY_MIGRATION.md`. The plain-CPM correction is implemented and qualified on master: one small CMake module uses ordinary CPM calls and its ordinary shared source cache for the exact private Cmajor/CHOC and official JUCE pins. The rejected custom source resolver, locks, repair, credential filtering, source receipts, and read-only enforcement are gone. Customer Cmajor command delivery is now implemented through `kit:setup`; its archive and installed-payload verification is separate from that source-dependency policy. Clean-customer qualification remains open.
...
- The private R2 bucket `builder-kit-feed` was verified earlier on September 3: the bucket-scoped check returned 78 objects totaling 169,550,604 bytes. The bucket-scoped credential does not grant account-wide bucket listing, so `rclone lsd r2:` returning `AccessDenied` is not evidence that this bucket is absent. No live R2/feed check was repeated during the later publication freeze.
- The private lineage repository `androidStern-personal/builder-kit-releases` exists. The earlier verified remote `master` and peeled `v0.1.0` tag both resolved to lineage commit `9eaa543531bb1e2acebd5ff393f363943481a2ef`; this integration run did not mutate that lineage.
...
- The reviewed Builder Kit source was fast-forwarded from `16fe62111d48019871d4a30d8a23426b9a71fad0` to remote master `b40b52aa3236774dc3d6cd4bdbc1fe804d39c26c`. The coordinator's clean worktree performed the push; the dirty primary checkout, its local master ref, and the user-owned SeqFX workshop were not moved or edited.
- Exact-source local qualification passed: direct `npm run typecheck` with zero diagnostics; `npm test` with 1,181 passed, zero failed, and one existing missing local Spectre shelf corpus skip; a 121-file committed customer export with canonical typecheck/test, Enhancer Lite build, and update-merge proof; and all seven effect-runtime builds. The previous `32aa72c7`/27-diagnostic/red-CI checkpoint is superseded. Master [CI run 33797774462](https://github.com/androidStern-personal/cosimo-synth/actions/runs/33797774462) completed successfully on the exact integrated source, including direct typecheck, setup/release safety contracts, the full tests, and customer-export proof.
- Independent review closed exact-commit export contamination, partial-publication ordering, inconsistent archive/payload tool identity, unverified JIT command selection, download-body secret diagnostics, and generated-plugin test omission. The final discovery rule excludes only `_browser.mjs`, so valid names such as `browser_gain` retain unit coverage. Retained Git packs preserve old source pins; publication remains serialized and cleanup separately authorized. Local payload receipts detect drift, not an adversary rewriting both receipt and payload.

Still open:

- Private `v0.1.1` publication remains open; no v0.1.1 tag, Keychain mutation, R2 publication, or old-prefix deletion occurred in this integration run. Publication is frozen pending Andrew's answer about whether anyone outside his own agents received the exposed v0.1.0 feed link. Rotation must either retire the old link after verification or retain it temporarily with an explicit customer migration path; retaining an unchanged old feed alone does not deliver v0.1.1 updates.
...
Portless 0.15.6 is installed and was proven as personal-only infrastructure; the version and default Node 22.16.0 were rechecked after source integration. Portless alone uses its separate Node 24 runtime. It is not a repository dependency, product dependency, customer prerequisite, or release gate; no repository script, Node pin, engine requirement, or configuration was added for it.

Bookkeeping exception: `TODOS.txt` is intentionally untouched because its authoritative primary-checkout copy contains user-owned changes. This roadmap and branch-local `PROGRESS.txt` record the separately authorized source integration and roadmap reconciliation; the stale planning lineage is not merged into master. The remaining private release and VirtualBuddy proof are separate gates. No public launch, DNS/Sites/Vercel changes, or outreach is authorized by this checkpoint.

## September 4, 2026 approved customer-experience cleanup

Andrew approved the following cleanup tranche and handoff to **Bob — Coordinate Builder Kit Daily Cleanup**, task `01a06784-1cfc-7f43-be19-5c93ade6f53a`. Bob owns implementation assignment, source review, integration, and delivery coordination; Woods owns this product specification. These are scoped follow-ups to the customer run, not authorization to execute the rest of this older launch roadmap.

Approved cleanup queue: **BK-21A, BK-13A, BK-13B, BK-22B, BK-22C, BK-40A**. Retain **BK-22A** as the existing investigation-only Spacebar task; do not duplicate it or silently broaden it into an approved repair.

- **Recommended first action after setup:** “Build and install the included plugin so I can try it in my DAW.” Build the existing example unchanged, then tell the customer the exact plugin name/location and invite their first requested modification. Do not make copying, renaming, generating new plugin/test source, or choosing a modification a prerequisite. Normal generated build output is expected. This is a recommended starting path, not a restriction on users who arrive with another explicit task.
- **Preset separation is shared-framework behavior.** Use the plugin's permanent identity for its saved-preset storage automatically, including when two plugins reuse the same UI module. Updating the same plugin keeps its presets; a different plugin does not silently share the example's saved-preset folder. Preserve existing saved presets. Adapting the shipped example to the shared behavior is in scope; an instruction telling each customer/agent to repair a hard-coded folder, or a special duplication scaffold, is not the solution.
- **Names and identity are separate.** Show the configured human-readable name in the DAW while allowing internal build filenames to remain space-free. Do not change stable host identifiers, sound parameters, DSP, or branding choices as a side effect.
- **Installation is an update when existing and replacement plugin identities match.** Compare the existing macOS bundle identifier and VST3 processor class identifier with the new build, not just their filenames. A mismatch or unreadable identity must stop before automatic replacement; failed installation must leave the old version recoverable. This is ordinary accidental-collision protection, not a new ownership/credential registry.
- **Browser UI development is optional.** The documented local browser entry point must work when chosen. It is neither a scheduled onboarding event nor a prerequisite for a native build. Do not add a hosted website, new browser-audio engine, or automatic preview launch to this cleanup.
- **Excluded:** private-feed logging/redaction work (Andrew said to ignore it); a duplicate-example command or a prescribed copying exercise; a new dirty-tree cleanup feature (the shipped Kit Update instructions already stop for uncommitted work); Skill Doctor, skill grading, or an unrelated instruction rewrite. Keep the existing explicit handling of uncommitted work.

Evidence anchor: the audited customer checkout `/Users/winterfell/src/builder-kit-playground` is release `0.1.1` at `9c4b1ac6b2219aa8aff17961e041b293cec92e5c`; build task `01a06b85-2ddd-78d1-9732-71e7cf8adc43`. The build/install succeeded, the test wrapper differed from the example only in its graph name, and copying it was an agent choice rather than a shipped requirement. The missing browser entry point was found by source inspection, not an observed failure in that run. No preset damage or accidental plugin replacement was observed. Do not edit the customer's playground or use its uncommitted experiment as the source implementation worktree.

Delivery boundaries: implement against current source master in coordinator-owned isolated worktrees, not this historical planning branch. Keep ordinary pinned CPM dependencies and ordinary Cmajor/CHOC fork commits; do not follow superseded dependency-provisioning instructions from this old checkout. Review source before broad gates, use focused regressions during repairs, parallelize independent work, and serialize native builds, installs, fixed ports, master mutations, and releases. Other worktrees and servers may be active; do not take over their ports or outputs. Validate the composed current customer export and approved first-use flow after relevant fixes land. Report automated qualification separately from actual DAW listening/keyboard acceptance. This handoff does not authorize a new feed release, unrelated plugin replacement, or public deployment.

## September 4, 2026 approved one-command installation — BK-21B

**COMPLETE — the corrective public bootstrap is live, the actual 153-character personalized command passed Woods' independent product check, and live isolated-host installation/rerun passed.** Bob (`01a06784-1cfc-7f43-be19-5c93ade6f53a`) owns engineering; Woods owns literal-command product review and follow-through. Andrew rejected the previous giant v0.1.2 shell line despite its technical tests; that superseded command is not the customer delivery. The private v0.1.2 release remains unchanged.

**Corrected user contract:** the entire customer command contains only an access-key export followed by downloading a short public bootstrap URL into Bash. Structural example only: `export ACCESS_KEY='<prefilled>'; curl -fsSL https://<public-short-url>/install.sh | bash`. The delivered key and URL are real and prefilled. All folder creation/selection, verification, temporary-file handling, failures, dependency setup and checks belong in hosted scripts, not in the clipboard command. The customer copies, pastes, and presses Enter: no placeholder editing, setup questionnaire, or follow-up commands.

**Literal-command product gate passed, including the actual delivered file:** Woods approved Bob's complete preview `export BUILDER_KIT_ACCESS='DUMMY_KEY'; curl -fsSL https://pub-2bb7a8a7b9b44ed3b975f3f0a6bcc756.r2.dev/install.sh | bash`. Only the key becomes personalized in the real delivery. The existing public R2 origin plus `/install.sh` is accepted; no extra launcher text may be appended after this review without another product check.

- **Public, short entry point:** publish a credential-free bootstrap script at a reasonably short HTTPS URL using the simplest suitable existing hosting. The script may fetch the existing private, verified installer/release using the exported key. Public means fetching the bootstrap itself does not need the buyer's key; it does not authorize exposing the customer kit, capability, private repository, or credentials. No new account/authentication system, marketing site, or unrelated DNS changes.
- **Reject disguised scripts:** no `bash -c`, nested quoting, `printf`, inline checksums/hash-addressed URLs, `mktemp`, traps, completion sentinels, encoded payloads, or repeated absolute destination paths in the pasted command. Folder creation goes inside the hosted script, using a safe default consistent with the existing fresh project destination and printing the result. Keep the existing preservation/refusal behavior.

- Reuse the existing delivery credential and verified-release/setup machinery. Do not invent a new token service, package manager, ownership registry, or commerce-page project. The bootstrap must work from outside a repository and must not depend on maintainer-local feed files, private Git access, or shared developer caches. It must not assume a project-local Node script can run before its required runtime exists.
- Make the exported credential available to both download and installer subprocesses. Keep populated customer credentials out of Git, diagnostic output, and test evidence; this is scoped handling of the new entry point, not reopening the deferred feed-logging audit.
- Run required kit setup and final checks automatically, with concise progress and a clear completion message naming the folder to open in Codex. Retain existing JUCE disclosure and explicit acknowledgment in the supplied command's surrounding notice; do not add a second command or questionnaire. Do not claim that setup grants a JUCE license or silently accept Apple/system agreements.
- Keep installation limited to preparing Builder Kit. Do not copy/rename examples, edit DSP, build/install a plugin, launch a browser or DAW, or modify another project as an implicit onboarding step. The approved first plugin-build request remains a separate customer action.
- Hosted download/authentication/setup stages must propagate failures and must never print ready after a failed step. The outer command uses ordinary `curl -fsSL ... | bash` behavior: failed public fetches must show the curl error and no success message; do not recreate the rejected inline program to preserve its special first-transfer exit-status/hash protocol. Refuse an unrelated occupied destination without changing it; preserve customer work on reruns and resume/skip completed setup safely. Report unavoidable OS prerequisites honestly.
- Update the actual customer delivery/setup instructions to supply this line. This supersedes older roadmap wording that requires an agent setup prompt or interactive folder/consent questions for initial installation; it does not authorize rewriting unrelated agent instructions.

**Acceptance, in this order:** (1) Bob sends Woods the complete proposed literal command with only the secret replaced by a dummy value, before expensive qualification or publication. Woods verifies that it really has the approved short shape. (2) Bob reviews source and runs focused failure/preservation tests for the changed entry point; retain useful existing installer proof rather than rerunning unrelated slow builds. (3) Publish the approved entry point and run the normal generated command against the real live delivery from a fresh isolated destination, without maintainer-only preparation, source/plugin changes or interactive setup input. Preserve Andrew's intended project destination. Verify safe rerun, invalid access/download failure, and unrelated-directory preservation. The approved isolated-host fallback remains valid if a VM is unavailable; label it honestly and keep earlier VM evidence separate. (4) Deliver the genuinely short prefilled command. Do not mark the product complete based only on commits, test counts, or publication. BK-40A consumes this path later; naming/Spacebar work stays separate.

**Decision provenance:** Andrew explicitly rejected the inline verification/launcher script and requested a short command pointing to publicly hosted Bash. The public first-stage script is trusted through its HTTPS publication origin, as in the requested curl-to-Bash pattern; downstream private installer/release verification remains inside hosted code. Do not claim this preserves the former clipboard-pinned first-stage hash check. Reuse the already tested installer rather than redesigning setup. Bob owns implementation and necessary release-version/test housekeeping without per-file approval loops; escalate genuine product/security/architecture choices, not routine fixture corrections. Woods must verify real progress after dispatch and drive through corrected delivery rather than requiring Andrew to keep restarting the work.

Delivery authority includes the already approved working-installer delivery and Andrew's explicit request to put the bootstrap online publicly: review/integrate/push the scoped correction and publish its secret-free bootstrap/private delivery. Preserve immutable v0.1.2; if changing a versioned artifact requires a new version, normal version and matching fixture maintenance are in scope. Do not broaden into a marketing launch, customer-code publication, unrelated DNS, global machine configuration or installed-plugin changes. This historical planning branch must not be merged into source master.

**September 4 completion evidence:** corrected source is integrated and pushed at `2ebd88eb6eea83cea04f0291301214b369fc0696`; source review and 71/71 integrated release-contract checks passed. Woods independently fetched public `/install.sh` without credentials (HTTP 200, 3,318 bytes, SHA-256 `ab2b2be033220524af40c967c6fef4aa148e54c8a183d3f6204dfc7d59eac373`) and checked the actual populated private command: exact approved shape, 153 characters/154 bytes including newline, one line, private mode 0600. Current private delivery is `/Users/winterfell/Library/Application Support/BuilderKitReleases/BK21B-short-20260904-6J3NCu/customer-delivery/delivery.txt` and `command.sh`; the old giant-line v0.1.2 delivery is superseded. Do not copy the populated credential into Git or public output.

Final live proof recorded 119 passing assertions, four command runs and one expected HTTP 404 observation (124 outcomes): installation exit 0 in 36 seconds, safe dirty rerun exit 0 in 4 seconds, invalid access and unrelated occupied destination both exit 1. Successful/rerun/occupied commands were byte-identical to the delivered file; only the test subprocess's `BUILDER_KIT_PROJECT_DIR` selected an isolated destination. Evidence is `completion.json`, `RELEASE_REPORT.md`, and `host-evidence/` under that same private delivery root. This is a fresh private host-folder proof, not a pristine OS/VM, default-home installation, or plugin/Ableton acceptance. Andrew's intended `/Users/winterfell/src/builder-kit-0.1.2` remains absent and untouched. The first plugin build remains a separate action.
...
| BK-13A | Separate saved presets automatically by plugin identity | A | Existing shared preset framework | **Approved for Bob, September 4, 2026.** Implement the September 4 shared-framework contract, and adapt the shipped example to consume it. A different plugin reusing the same UI gets separate saved-preset storage without customer instructions or a special scaffold. Same-plugin rebuild/update retains presets. Preserve existing saved presets and current sound/state behavior; do not blindly migrate one shared folder into every derivative. Prove two identities using the same UI remain separate, same-identity upgrades retain presets, and the original example's existing presets remain available without deletion. | Scoped framework cleanup |
| BK-13B | Preserve human-readable plugin names in the DAW | A | Existing product identity/build pipeline | **Approved for Bob, September 4, 2026.** Carry the configured display name, including spaces, into the generated plugin's host-visible metadata. Internal build/output filenames may remain identifier-shaped. Preserve bundle identifiers, VST3 class identity, parameter identities, and source manifests. Fix the owning framework/generator layer; if Cmajor owns the defect, use an ordinary fork commit and pin update, not a generated-file splice. Prove the emitted name and unchanged identity in a real built artifact; report actual Ableton display acceptance separately. | Scoped naming cleanup |
...
| BK-21A | Print a complete copy-paste command for JUCE acknowledgment | A | BK-21 | **Approved for Bob, September 4, 2026; replaces the earlier backlog-only hold.** Replace flag-only instructions such as `JUCE terms: not acknowledged; pass --accept-juce-terms to continue` with plain language: "Read the JUCE licensing notice above. If you agree, run this command from your Builder Kit project folder:" followed on its own line by `npm run kit:setup -- --accept-juce-terms`. Apply consistently to the setup preview and missing-acknowledgment error. Preserve explicit consent and the existing licensing requirement; never auto-accept. Add focused output assertions for the full command. No unrelated setup redesign or release required. | Small CLI cleanup |
...
| BK-22A | Investigate compiled-plugin Spacebar capture and shared host-keyboard regression | A | BK-22 | **Queued investigation, reported September 4, 2026; no repair authorized by this ticket.** Andrew reports that his built plugin swallows Space instead of passing it to Ableton transport; he expects the existing framework-level fix to cover every plugin automatically. Establish the exact installed plugin, build/source/toolchain identity, Ableton version, and failing focus/gesture state before assigning cause. Compare the last working path with the customer-built dedicated VST3; trace shared DOM keyboard ownership, document-start routing, original native keyDown/keyUp forwarding, and wrapper/CHOC inclusion. Cover unfocused UI, knobs, numeric fields, actual text editing, menus, and active drags; distinguish legitimate text entry from accidental host-key capture. Report the reproducer, root cause and regression boundary (or explicit remaining uncertainty), why existing guards/tests missed it, and the smallest shared-framework prevention plus focused regression/real-Ableton proof plan. Do not presume a per-plugin fix or call binary marker checks/browser tests host acceptance. Preserve installed plugins and live Ableton work; no uncoordinated rebuild/install, global focus hack, or customer-playground edits. | Investigation backlog |
| BK-22B | Replace only the intended plugin and retain install recovery | A | Existing dedicated VST3 installer | **Approved for Bob, September 4, 2026.** Preflight existing and new plugin identities using their bundle and VST3 processor class identifiers, not just the target filename. Normal same-plugin updates remain supported; conflicting or unreadable existing identity stops before mutation and is explained. Stage/validate the replacement and keep the prior installation recoverable until success, with rollback on failure. Use the existing identity/build pipeline, not a new ownership registry. Focused tests cover first install, same-plugin update, same-name/different-identity collision, unreadable metadata, and failed installation recovery using isolated install fixtures. Do not overwrite Andrew's live plugins to run those tests. | Scoped installer cleanup |
| BK-22C | Make the optional local browser UI entry point work | A | Existing shared UI/dev pipeline | **Approved for Bob, September 4, 2026.** Repair the documented browser-development path so both the included example and an ordinary generated starter can load their real plugin UI through the shared development tooling. Test the exact documented entry path from the exported customer project, not only the separate test-shell page. Preserve the production UI and normal framework bindings; do not build a substitute demonstration UI. Keep preview opt-in, with no mandatory onboarding/build step, hosted website, or new browser-audio engine. State audio availability truthfully. Respect existing servers/worktrees; never seize an occupied port. | Scoped dev-flow cleanup |
...
| BK-40A | Provide and prove the approved first-use starting prompt | A/H | BK-21A, BK-13A/B, BK-22B for final composed proof | **Approved for Bob, September 4, 2026.** After setup, recommend: "Build and install the included plugin so I can try it in my DAW." Give one concise customer entry point that uses the existing included plugin unchanged; do not require copying, renaming, new plugin/test source files, a browser preview, or a predetermined modification. Build-generated files are normal. On success name the actual DAW plugin/location, then invite the customer's chosen first modification; users with another explicit task may skip this recommended path. Prove the exact prompt/flow against the current exported kit without personal memories, global private instructions, or Cosimo-monorepo access, distinguishing automated build proof from human listening. No branded onboarding application or documentation suite is authorized by this ticket. | Small first-use flow |
