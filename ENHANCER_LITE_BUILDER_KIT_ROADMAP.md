...
**Approved for implementation; not delivered yet.** Bob (`01a06784-1cfc-7f43-be19-5c93ade6f53a`) owns engineering. This is the next customer-entry follow-up, replacing the long manual bootstrap/setup recipe before Andrew is asked to test installation again.

**User contract:** one personalized, ready-to-paste shell line exports the existing access credential first, creates and enters the project folder, then downloads and runs the installer (`curl ... | bash` style). The real credential, destination, and download address are already filled in. The customer copies, pastes, and presses Enter: no placeholder editing, setup questionnaire, or follow-up Git, setup, doctor, or acknowledgment commands.
...
- Run required kit setup and final checks automatically, with concise progress and a clear completion message naming the folder to open in Codex. Retain existing JUCE disclosure and explicit acknowledgment: explain it alongside the supplied command and include the acknowledgment option there, rather than asking for a second command. Do not claim that setup grants a JUCE license or silently accept Apple/system agreements.
...
- Make download/authentication/setup failures propagate as failures, including through the shell pipeline. Never print ready after a failed step. Refuse an unrelated occupied destination without changing it; preserve customer work on reruns and resume/skip completed setup safely. On a machine missing an unavoidable OS prerequisite, report the precise unmet requirement rather than claiming the one-command path passed.
...
**Acceptance:** Bob must exercise the exact supplied one-liner from an arbitrary starting directory in a clean supported customer environment, without maintainer-only preparation or interactive setup input. Prove successful setup, safe rerun, invalid access/download failure, and unrelated-directory preservation. Record actual prerequisite requirements and separate automated proof from DAW acceptance. BK-40A should consume this installation path for its subsequent first-plugin proof; do not ask Andrew to repeat the old recipe or quietly broaden the held naming/Spacebar work.

**Decision provenance:** Andrew explicitly chose a prefilled terminal command over Woods's suggested interactive installer. The command owns folder selection and credential setup; the script owns the remaining kit setup/checks. Reusing existing setup machinery keeps this a bounded entry-point change. Bob chooses low-level scripting/delivery details and reports any genuine conflict with the approved no-extra-steps experience before substituting another flow.

Delivery authority remains source review, focused tests, clean-environment proof, and coordinator integration/push of the approved source changes. A new feed release, public deployment, or changes to Andrew's installed plugins still require separate authorization. This historical planning branch must not be merged into source master.
