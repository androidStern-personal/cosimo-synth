...
**REOPENED — v0.1.2 was published and technically tested, but Andrew rejected its giant shell line. The short-command product requirement is not complete.** Bob (`01a06784-1cfc-7f43-be19-5c93ade6f53a`) owns engineering; Woods owns literal-command product review and active follow-through. Preserve the existing release and its qualification evidence; fix the customer entry point without pretending that one physical line is the same as a short command.
...
**Literal-command product gate passed (publication/live proof still pending):** Woods approved Bob's complete preview `export BUILDER_KIT_ACCESS='DUMMY_KEY'; curl -fsSL https://pub-2bb7a8a7b9b44ed3b975f3f0a6bcc756.r2.dev/install.sh | bash`. Only the key becomes personalized in the real delivery. The existing public R2 origin plus `/install.sh` is accepted; no extra launcher text may be appended after this review without another product check.
