...
Ableton Live 11.3.43's scanner discovered the audition VST3 with the correct ID, `Fx` class, version `0.2.0`, and installed path. Editor-load acceptance was not completed. A same-bundle second-instance attempt could not be addressed independently by Computer Use; the quit/discard action reached the pre-existing unsaved `Untitled` AbletonMCP test session instead. That session exited, so the requirement to leave it undisturbed failed. The saved `Sweden.als` and its backups retained their prior timestamps, but no further Ableton action was attempted after the process-routing failure.
