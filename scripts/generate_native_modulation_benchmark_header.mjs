...
function requireCommonSourceContract(events) {
    ...
    const envelopes = events.filter((event) => event.endpointID === "modulationEnvelope");
    if (msegBuffers.length !== 6 || msegPlaybacks.length !== 3 || envelopes.length !== 3) {
        throw new Error("Neutral profile no longer installs the expected 6 MSEG buffers, 3 playbacks, and 3 envelopes");
    }
    ...
    return { msegPlaybacks, envelopes };
}
...
function generateHeader() {
    ...
    const { msegPlaybacks, envelopes } = requireCommonSourceContract(referenceCommonEvents);
    ...
    const envelopeRows = envelopes.map(({ value }) => (
        `    { ${cppInt(value.slot)}, ${cppFloat(value.attackSeconds)}, ${cppFloat(value.decaySeconds)}, ${cppFloat(value.sustain)}, ${cppFloat(value.releaseSeconds)} }`
    )).join(",\n");
...
inline constexpr std::array<Envelope, ${envelopes.length}> envelopes {{
${envelopeRows}
}};
