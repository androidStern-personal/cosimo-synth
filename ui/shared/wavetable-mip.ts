...
const bitReverseIndexCache = new Map<number, Uint32Array>();

function getBitReverseIndices(size: number) {
...
function fftComplexInPlace(real: Float64Array, imaginary: Float64Array, inverse = false) {
    const size = real.length;

    assert(size === imaginary.length, "FFT real and imaginary buffers must have the same length");
    assert(isPowerOfTwo(size), "FFT input length must be a power of two");
...
export function canonicalizeFrame(frame: ArrayLike<number>) {
...
export function extractSourceFramesFromSamples(
    samples: ArrayLike<number>,
    {
        ...
        maxFramesPerTable?: number;
    } = {},
) {
    const sourceSamples = Float32Array.from(samples);

    assert(sourceSamples.length % samplesPerFrame === 0, `Source wavetable files must contain a whole number of ${samplesPerFrame}-sample frames`);
    ...
    assert(frameCount <= maxFramesPerTable, `Source wavetable files must contain at most ${maxFramesPerTable} frames`);
...
export function buildFrameSpectrum(frame: ArrayLike<number>): Spectrum {
...
export function buildMipFrameFromSpectrum(
    ...
    assert(size === spectrum.imaginary.length, "Spectrum real and imaginary buffers must have the same length");
    assert(mipIndex >= 0 && mipIndex < mipLevelCount, `Mip index must stay inside [0, ${mipLevelCount - 1}]`);
...
export function buildMipFrameFromFrame(frame: ArrayLike<number>, mipIndex: number, options: { mipLevelCount?: number } = {}) {
    return buildMipFrameFromSpectrum(buildFrameSpectrum(frame), mipIndex, options);
}

export function buildSpectrumCacheForFrames(frames: ArrayLike<number>[]) {
