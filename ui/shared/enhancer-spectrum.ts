...
export type EnhancerSpectrumDisplay = {
...
function normalizeSpectrumMessage(message: unknown): EnhancerSpectrumFrame | null {
...
function sampleFrame(frame: EnhancerSpectrumFrame): ReadonlyArray<number> {
...
function smoothSpectrum(
    previous: EnhancerSpectrumDisplay | null,
...
function findPeakDbfs(magnitudesDbfs: ReadonlyArray<number>): number {
    let peak: number = ENHANCER_SPECTRUM_PLOT.minimumLevelDbfs;
...
export function enhancerFrequencyX(frequencyHz: number): number {
...
export function enhancerFrequencyFromX(plotX: number): number {
    const plotWidth = ENHANCER_SPECTRUM_PLOT.width
        - ENHANCER_SPECTRUM_PLOT.left
        - ENHANCER_SPECTRUM_PLOT.right;
    const normalized = clamp(
        (plotX - ENHANCER_SPECTRUM_PLOT.left) / plotWidth,
        0,
        1,
    );
...
export function enhancerFrequencyClientX(
    frequencyHz: number,
    renderedRect: EnhancerRenderedRect,
): number {
...
export function enhancerFrequencyFromClientX(
    clientX: number,
    renderedRect: EnhancerRenderedRect,
): number {
 ...
 */
export function enhancerFrequencyAfterClientDrag(
    ...
    return enhancerFrequencyFromClientX(
...
export function formatEnhancerFrequencyTick(frequencyHz: number): string {
 ...
 */
export function enhancerFrequencyTicksForWidth(
    renderedWidth: number,
): ReadonlyArray<EnhancerFrequencyTick> {
...
export function enhancerGainY(gainDb: number): number {
    const normalized = (
        clamp(
            gainDb,
            ENHANCER_SPECTRUM_PLOT.minimumGainDb,
            ENHANCER_SPECTRUM_PLOT.maximumGainDb,
        ) - ENHANCER_SPECTRUM_PLOT.minimumGainDb
    ) / (
        ENHANCER_SPECTRUM_PLOT.maximumGainDb
        - ENHANCER_SPECTRUM_PLOT.minimumGainDb
 ...
 */
export function enhancerBellResponseDb(
    frequencyHz: number,
    centerHz: number,
    q: number,
    amount: number,
...
export function enhancerLevelY(levelDbfs: number): number {
    const normalized = (
        clamp(
            levelDbfs,
...
export function advanceEnhancerSpectrum(
    ...
    const path = createEnhancerFrequencyPath((frequencyHz) => {
        const index = Math.round(
            Math.log(frequencyHz / ENHANCER_SPECTRUM_PLOT.minimumHz)
            / Math.log(
                ENHANCER_SPECTRUM_PLOT.maximumHz / ENHANCER_SPECTRUM_PLOT.minimumHz,
            )
