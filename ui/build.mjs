...
async function emitGeneratedPatchGuiModule(sourceRelativePath, outputRelativePath, { bundle = false } = {}) {
    const sourceFile = path.join(repoRoot, sourceRelativePath);
    const outputFile = path.join(repoRoot, outputRelativePath);
    // Adapters with real transitive dependencies need a self-contained module;
    // moving their relative imports beside patch_gui would change resolution.
    if (bundle) {
        const result = await bundleModule({ entryPoints: [sourceFile], bundle: true,
            write: false, format: "esm", platform: "browser", target: "es2022" });
        await writeTextFileIfChanged(outputFile,
...
if (shouldBuild("--desktop") || shouldBuild("--desktop-runtime") || shouldBuild("--ios")) {
    ...
    await emitGeneratedPatchGuiModule("ui/shared/enhancer-spectrum.ts", "patch_gui/enhancer-spectrum.js");
