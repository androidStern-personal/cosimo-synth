...
const SEQFX_PATTERN_COUNT = 12;
const SEQFX_PARAM_COUNT = 8;
const SEQFX_LANES = {
  filter: 0,
  crusher: 1,
  tapeStop: 2,
...
const SEQFX_LANE_NAMES = [
  "Filter",
  "Crusher",
  "Tape Stop",
  "Stutter"
];
const DEFAULT_LANE_PARAMS = [
  [0, 2e3, 500, 0.707, 1, 0, 0, 0],
  [8, 1, 0, 0, 0, 0, 0, 0],
  [1, 1, 0, 30, 0, 0, 0, 0],
  [8, 1, 0, 0, 0, 0, 0, 0]
];
...
const INTEGER_PARAMS = /* @__PURE__ */ new Set([
...
const TRIGGER_LATCHED_PARAMS = /* @__PURE__ */ new Set([
...
function clampIndex(value, maxExclusive, label) {
...
function normalizeParam(lane, paramIndex, value) {
...
function isSeqFxTriggerLatchedParam(lane, paramIndex) {
  return TRIGGER_LATCHED_PARAMS.has(`${lane}:${paramIndex}`);
}
function createDefaultStep(lane) {
  return {
    active: false,
    trigger: false,
    mix: 1,
...
function createDefaultSeqFxState() {
  return {
    version: 1,
...
function normalizeStep(candidate, lane) {
  const fallback = createDefaultStep(lane);
  const step = candidate && typeof candidate === "object" ? candidate : {};
  ...
  return {
    ...
    params: Array.from({ length: SEQFX_PARAM_COUNT }, (_unused, paramIndex) => normalizeParam(lane, paramIndex, Number(rawParams[paramIndex] ?? fallback.params[paramIndex])))
  };
}
function normalizePattern(candidate) {
  const pattern = candidate && typeof candidate === "object" ? candidate : {};
  ...
  return {
    ...
    lanes: Array.from({ length: SEQFX_LANE_COUNT }, (_unused, lane) => {
      ...
      const rawSteps = rawLane && typeof rawLane === "object" && Array.isArray(rawLane.steps) ? rawLane.steps : [];
      return {
        steps: Array.from({ length: SEQFX_STEP_COUNT }, (_unusedStep, step) => normalizeStep(rawSteps[step], lane))
      };
    })
  };
}
function normalizeSeqFxState(candidate) {
  ...
  const rawState = candidate && typeof candidate === "object" ? candidate : {};
  const rawPatterns = Array.isArray(rawState.patterns) ? rawState.patterns : [];
  return {
    version: 1,
    patterns: Array.from({ length: SEQFX_PATTERN_COUNT }, (_unused, pattern) => normalizePattern(rawPatterns[pattern]))
  };
}
function serializeSeqFxState(state) {
  return JSON.stringify(normalizeSeqFxState(state));
}
function withEditedPattern(state, patternIndex, edit) {
...
function normalizeBlockLength(startStep, length) {
  const maximum = SEQFX_STEP_COUNT - startStep;
  return Math.min(maximum, Math.max(1, Math.trunc(Number(length)) || 1));
}
function stepIsInsideRange(step, startStep, length) {
  return step >= startStep && step < startStep + length;
}
function getBlockForStep(pattern, lane, step) {
  const laneSteps = pattern.lanes[lane]?.steps;
  if (!laneSteps?.[step]?.active) {
    return null;
  }
  let startStep = step;
  while (startStep > 0 && laneSteps[startStep].active && !laneSteps[startStep].trigger && laneSteps[startStep - 1].active) {
    startStep -= 1;
  }
  let endStep = startStep;
  while (endStep + 1 < SEQFX_STEP_COUNT && laneSteps[endStep + 1].active && !laneSteps[endStep + 1].trigger) {
    endStep += 1;
  }
  return {
    lane,
...
function clearBlock(pattern, lane, block) {
...
function writeBlock(pattern, lane, startStep, length, template) {
...
function getSeqFxLaneBlocks(pattern, lane) {
...
function getSeqFxBlockAtStep(pattern, lane, step) {
...
function applySeqFxBlockCreate(state, edit) {
...
function applySeqFxBlockResize(state, edit) {
  return withEditedPattern(state, edit.patternIndex, (pattern) => {
    const lane = clampIndex(edit.lane, SEQFX_LANE_COUNT, "lane");
...
function applySeqFxBlockMixEdit(state, edit) {
  return withEditedPattern(state, edit.patternIndex, (pattern) => {
    const lane = clampIndex(edit.lane, SEQFX_LANE_COUNT, "lane");
    const startStep = clampIndex(edit.startStep, SEQFX_STEP_COUNT, "startStep");
    const block = getBlockForStep(pattern, lane, startStep);
...
function applySeqFxBlockParamEdit(state, edit) {
  return withEditedPattern(state, edit.patternIndex, (pattern) => {
    const lane = clampIndex(edit.lane, SEQFX_LANE_COUNT, "lane");
    const startStep = clampIndex(edit.startStep, SEQFX_STEP_COUNT, "startStep");
...
function applySeqFxCellToggle(state, edit) {
  return withEditedPattern(state, edit.patternIndex, (pattern) => {
    const lane = clampIndex(edit.lane, SEQFX_LANE_COUNT, "lane");
...
function applySeqFxMixEdit(state, edit) {
...
function applySeqFxParamEdit(state, edit) {
  const lane = clampIndex(edit.lane, SEQFX_LANE_COUNT, "lane");
  const paramIndex = clampIndex(edit.paramIndex, SEQFX_PARAM_COUNT, "paramIndex");
  const steps = normalizeSteps(edit.steps);
...
function buildSeqPatternUpload(state, options) {
...
function toEchoToken(value) {
  return typeof value === "string" ? value : JSON.stringify(value);
}
function resolvePatternIndex(value) {
...
class SeqFxRuntimeBridge {
  ...
  requestBootState() {
    ...
    if (!sawFullState) {
      this.applyStoredState(void 0, false);
    }
  ...
  commitState(nextState, editedPatternIndex) {
  ...
  uploadSelectedPattern(authoritative) {
...
const PARAM_DEFINITIONS = {
  [SEQFX_LANES.filter]: [
    ...
    { index: 1, label: "Start cutoff", min: 20, max: 2e4, step: 1 },
    { index: 2, label: "End cutoff", min: 20, max: 2e4, step: 1 },
...
function selectionFromCell(cell) {
  return cell ? { lane: cell.lane, steps: [cell.step] } : null;
}
function mergeRangeSelection(anchor, target) {
...
function SeqFxPatchView({ patchConnection }) {
  ...
  const laneTrackStyle = reactExports.useMemo(() => ({
    minWidth: `${trackWidth}px`,
    height: `${cellSize}px`
  }), [cellSize, trackWidth]);
  ...
  const inspectedCell = inspectedLane !== null && inspectedStep !== null ? selectedPatternState.lanes[inspectedLane].steps[inspectedStep] : null;
  const inspectedBlock = inspectedLane !== null && inspectedStep !== null ? getSeqFxBlockAtStep(selectedPatternState, inspectedLane, inspectedStep) : null;
  const selectedWholeBlock = Boolean(
    activeSelection && inspectedBlock && activeSelection.lane === inspectedBlock.lane && activeSelection.steps.length === inspectedBlock.length && activeSelection.steps[0] === inspectedBlock.startStep
  );
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("main", { className: gestureState ? "seqfx-root is-dragging" : "seqfx-root", "data-role": "seqfx-root", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "seqfx-topbar", "aria-label": "SeqFX transport and pattern controls", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "seqfx-title", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { children: "SeqFX" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "seqfx-patterns", role: "group", "aria-label": "Patterns", children: Array.from({ length: SEQFX_PATTERN_COUNT }, (_unused, index) => /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "seqfx-transport", role: "group", "aria-label": "Internal clock", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", onClick: () => bridge.playInternal(), children: "Play" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", onClick: () => bridge.stopInternal(), children: "Stop" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", onClick: () => bridge.resetInternal(), children: "Reset" })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "seqfx-workspace", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "seqfx-grid-shell", "aria-label": "Effect sequence grid", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "seqfx-step-header", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "seqfx-lane-spacer" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "seqfx-step-track", ref: measureTrackRef, style: stepTrackStyle, children: STEP_NUMBERS.map((step) => /* @__PURE__ */ jsxRuntimeExports.jsx(
            "div",
            {
        ...
        SEQFX_LANE_NAMES.map((laneName, lane) => {
          ...
          return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "seqfx-lane-row", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "seqfx-lane-label", children: laneName }),
              ...
              "div",
              {
                ...
                children: [
                  ...
                  laneBlocks.map((block) => {
                    const selected = activeSelection?.lane === lane && activeSelection.steps[0] === block.startStep && activeSelection.steps.length === block.length;
                    const className = [
                      "seqfx-block",
                      selected ? "is-selected" : "",
                      playheadStep !== null && playheadStep >= block.startStep && playheadStep <= block.endStep ? "is-playhead" : ""
                    ].filter(Boolean).join(" ");
                    const ariaLabel = block.length === 1 ? `${laneName} block ${block.startStep + 1}` : `${laneName} block ${block.startStep + 1}-${block.endStep + 1}`;
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("aside", { className: "seqfx-inspector", "data-role": "seqfx-inspector", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "seqfx-inspector-heading", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Inspector" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: getSelectionLabel(activeSelection) })
        ] }),
        !inspectedCell || inspectedLane === null ? /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "seqfx-empty", children: "Choose a lane cell to edit its mix and effect settings." }) : /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "seqfx-field", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Mix" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "input",
              {
                "data-role": "seqfx-mix",
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx("output", { children: formatValue(inspectedCell.mix) })
          ] }),
          PARAM_DEFINITIONS[inspectedLane].map((definition) => {
            ...
            return /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "seqfx-field", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
                definition.label,
                triggerLatched ? /* @__PURE__ */ jsxRuntimeExports.jsx("em", { children: "Trigger" }) : null
              ] }),
              definition.kind === "select" ? /* @__PURE__ */ jsxRuntimeExports.jsx(
                "select",
                {
                  ...
                  children: definition.options.map((option, index) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: index, children: option }, option))
                }
              ) : /* @__PURE__ */ jsxRuntimeExports.jsx(
                "input",
                {
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx("small", { children: disabled ? "Select one cell to edit this trigger." : definition.hint ?? `${definition.min} to ${definition.max}` })
            ] }, definition.index);
          }),
          selectedWholeBlock && inspectedBlock ? /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "seqfx-chain", "aria-label": "Fixed signal path", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Filter" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Crusher" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Tape" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Stutter" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Mix" })
        ] })
      ] })
    ] }),
...
class SeqFxErrorBoundary extends reactExports.Component {
  state = {
    errorMessage: null
  };
...
class SeqFxPatchViewElement extends HTMLElement {
  ...
  renderApp() {
    ...
    this.root.render(
      /* @__PURE__ */ jsxRuntimeExports.jsx(SeqFxErrorBoundary, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(SeqFxPatchView, { patchConnection: this.patchConnection }) })
    );
  }
}
function getTagName() {
...
function createSeqFxPatchView(patchConnection) {
