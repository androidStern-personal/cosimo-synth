...
class OttLabView extends HTMLElement {
  constructor(patchConnection) {
    ...
    this.snapshotController = new EffectSnapshotBankController({
      effectID: "ott",
      patchConnection,
      slotIDs: SNAPSHOT_SLOT_IDS,
      legacyBankProvider: ({ fullStoredState } = {}) => createLegacySnapshotBank({
        snapshotStore: this.snapshotStore,
        activeSlotID: fullStoredState?.[ACTIVE_SNAPSHOT_SLOT_STATE_KEY],
      }),
    });
...
function createLegacySnapshotBank({ snapshotStore, activeSlotID }) {
  if (!snapshotStore)
    return null;

  if (snapshotStore.__loadError)
    throw new Error(snapshotStore.__loadError);

  const bank = createEmptyEffectSnapshotBank({
    effectID: "ott",
    slotIDs: SNAPSHOT_SLOT_IDS,
  });

  for (const slotID of SNAPSHOT_SLOT_IDS)
    bank.slots[slotID] = snapshotStore.slots[slotID] ?? null;

  const normalizedActiveSlotID = normalizeSnapshotSlotID(activeSlotID);
  bank.activeSlotID = normalizedActiveSlotID && bank.slots[normalizedActiveSlotID]
    ? normalizedActiveSlotID
    : null;
