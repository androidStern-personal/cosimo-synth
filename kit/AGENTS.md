...
- `cmaj` is obtained only through `kit:setup` from the pinned toolchain. `fx:prod:build` resolves it from `build/kit-tools/cmaj` when that binary matches the hash in `kit/toolchain.json` (a checkout that carries the Cmajor command-tool source may fall back to its own pinned build) and otherwise fails with a clear error naming `npm run kit:setup`. Do not install a different `cmaj` and do not point the build at one: the pin, the Cmajor source commit in `kit/cmake/CosimoDependencies.cmake`, and the patched generic `CmajPlugin.vst3` must all match.
