...
import type { EffectPreset } from "./effect-preset-shared";
import {
...
export type StandaloneEffectPresetControllerOptions = {
    ...
    /** Successful sound replacements (preset/import applies, and the synth adapter's Init/share/bounce); presentation-only audition state may reset here. */
...
/** A sound replacement prepared but not yet run; adapters may defer it behind an unsaved-work decision. */
export type PreparedSoundReplacement<T = unknown> = {
...
export function valuesEqual(left: EffectParameterValue | undefined, right: EffectParameterValue) {
    return Object.is(left, right);
}

export function storedStateValuesEqual(left: unknown, right: unknown) {
    return canonicalJSONStringify(left) === canonicalJSONStringify(right);
}

export function presetKeyFor(source: StandaloneEffectPresetSource, presetID: string) {
...
export function ensureStoredStateWriter(patchConnection: PatchConnectionLike, operation: string) {
...
export function ensureParameterWriter(patchConnection: PatchConnectionLike, operation: string) {
...
export function defaultParameterValues(contract: EffectPluginStateContract) {
...
export class StandaloneEffectPresetController {
    protected readonly bridge: EffectPresetRuntimeBridgeV2;
    private readonly factoryPresetRegistry: Record<string, StandaloneEffectFactoryPreset[]>;
    protected readonly storedStateAdapters: EffectStoredStateAdapter[];
    ...
    protected readonly currentValues = new Map<string, EffectParameterValue>();
    ...
    private ready = false;
    protected applyingPresetValuesDepth = 0;
    private loadGeneration = 0;
    protected lastError: string | null = null;

    constructor(protected readonly options: StandaloneEffectPresetControllerOptions) {
        ...
        this.factoryPresetRegistry = options.factoryPresets ?? {};
    ...
    getState(): StandaloneEffectPresetState {
        ...
        return {
            ...
            activeLabel: activePreset?.label ?? this.getUnnamedLabel(),
            dirty: activePreset?.dirty ?? this.getUnnamedDirty(),
            ...
            supportsInit: this.supportsInit(),
            pendingSoundReplacement: this.getPendingSoundReplacement(),
        };
    }

    // ── Extension seams ──────────────────────────────────────────────────
    // A host-specific adapter (e.g. the Cosimo synth's) subclasses the
    // controller and overrides these to add an unnamed working identity and a
    // guarded sound-replacement policy. The generic controller has neither.

    /** Label shown while no preset is active. */
    protected getUnnamedLabel(): string {
        return "";
    }

    /** Dirty flag while no preset is active. */
    protected getUnnamedDirty(): boolean {
        return false;
    }

    /** Whether the host exposes a canonical Init command. */
    protected supportsInit(): boolean {
        return false;
    }

    /** The sound replacement waiting on an unsaved-work decision, if any. */
    protected getPendingSoundReplacement(): StandaloneEffectPendingSoundReplacement | null {
        return null;
    }

    /** A parameter edit arrived while no preset is active. */
    protected handleUnnamedParameterEdit(_endpointID: string, _value: EffectParameterValue): void {}

    /** A stored-state edit arrived while no preset is active. */
    protected handleUnnamedStoredStateEdit(_key: string, _serializedValue: unknown): void {}

    /** A user preset was captured from the live sound (save or overwrite). */
    protected recordCleanCapturedPreset(_preset: EffectPresetV2): void {}
    ...
    reapplyActivePreset(): StandaloneEffectPresetMutationResult<EffectPresetV2> {
        ...
        if (!activePreset) {
            return this.reapplyWithoutActivePreset();
        }
    ...
    /** Extension seam: reapply pressed while no preset is active (e.g. a synth adapter's unnamed-sound baseline). */
    protected reapplyWithoutActivePreset(): StandaloneEffectPresetMutationResult<EffectPresetV2> {
    ...
    saveCurrentAsNewPreset(label: string): StandaloneEffectPresetMutationResult<EffectPresetV2> {
        return this.runMutation(() => {
            ...
            this.recordCleanCapturedPreset(preset);
    ...
    overwriteUserPreset(presetKey: string): StandaloneEffectPresetMutationResult<EffectPresetV2> {
        return this.runMutation(() => {
            ...
            this.recordCleanCapturedPreset(nextPreset);
    ...
    protected attachStoredStateListeners() {
    ...
    private markActivePresetDirtyIfNeeded(endpointID: string, value: EffectParameterValue) {
        ...
        if (!activePreset) {
            this.handleUnnamedParameterEdit(endpointID, value);
            return;
        }
    ...
    private markActivePresetDirtyForStoredStateIfNeeded(adapter: EffectStoredStateAdapter) {
        ...
        if (!activePreset) {
            this.handleUnnamedStoredStateEdit(adapter.key, currentStoredState);
            return;
        }
    ...
    protected requireCurrentContract() {
    ...
    protected captureCurrentPreset(presetID: string, label: string) {
    ...
    protected normalizePresetForCurrentContract(preset: unknown) {
    ...
    protected commitActivePresetAndApply(preset: EffectPresetV2) {
    ...
    protected commitImportedPresetAndApply(preset: EffectPresetV2) {
        const previousState = this.bridge.getState();
        this.bridge.saveUserPreset(preset, { activate: true });
    ...
    protected applyPresetValuesToPatch(preset: EffectPresetV2, options: ApplyPresetValuesOptions) {
    ...
    protected nextLoadGeneration() {
        this.loadGeneration += 1;
        return this.loadGeneration;
    }

    protected queueSuppressedParameterValue(
        endpointID: string,
    ...
    protected removeSuppressedParameterValue(endpointID: string, suppression: SuppressedParameterValue) {
        const queue = this.suppressedParameterValues.get(endpointID);
    ...
    /**
     * Extension seam: run a prepared sound replacement now, or defer it. The
     * generic controller never defers; a synth adapter may park it behind an
     * unsaved-work confirmation instead.
     */
    protected requestSoundReplacement<T>(
        replacement: PreparedSoundReplacement<T>,
    ): StandaloneEffectPresetMutationResult<T> {
        return this.runSoundReplacement(replacement);
    }

    protected runSoundReplacement<T>(
        replacement: PreparedSoundReplacement<T>,
    ...
    protected fail<T = never>(error: Error): StandaloneEffectPresetMutationResult<T> {
