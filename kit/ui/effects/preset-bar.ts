...
/**
 * Sound-replacement continuations, looked up by duck-typing a controller's
 * optional `getSynthMutations` so the generic bar can drive the dirty-guard
 * dialog for controllers that gate sound replacement (the synth adapter)
 * without importing any synth module.
 */
export type StandaloneSoundReplacementMutations = {
...
export const DEFAULT_PRESET_BAR_ELEMENT_NAME = "cosimo-preset-bar";

export class PresetBar extends HTMLElement {
    private _controller: StandaloneEffectPresetController | null = null;
    private _unsubscribe: (() => void) | null = null;
    protected _state: StandaloneEffectPresetState | null = null;
    protected _mutations: ReturnType<StandaloneEffectPresetController["getMutations"]> | null = null;
    protected _synthMutations: StandaloneSoundReplacementMutations | null = null;
    ...
    protected _dialogContinuesSoundReplacement = false;

    // Cached DOM refs
    protected _els!: Record<string, HTMLElement>;

    private readonly _handleDocumentPointerDown = (event: PointerEvent) => {
        if (
            !this._flyoutOpen
            && !this._hasOpenExtensionSurface()
    ...
    set controller(next: StandaloneEffectPresetController | null) {
        ...
        if (next) {
            ...
            const synthCapable = next as {
                getSynthMutations?: () => StandaloneSoundReplacementMutations | null;
            };
    ...
    // ── Extension seams ──────────────────────────────────
    // Overridden by presentation extensions (e.g. the synth shell bar) that
    // register additional surface under the same element registration.

    /** Reset extension state whenever the controller changes (including to null). */
    protected _prepareController(_next: StandaloneEffectPresetController | null): void {}

    /** Whether an extension-owned transient surface (menu, popover) is open. */
    protected _hasOpenExtensionSurface(): boolean {
        return false;
    }

    /** Close any extension-owned transient surfaces. */
    protected _closeExtensionSurfaces(): void {}

    /** Runs after an action element was activated, before the click finishes. */
    protected _afterActionActivated(_actionEl: HTMLElement): void {}

    /** Runs on every state notification, before the generic bar re-renders. */
    protected _syncExtensionSurfaces(_state: StandaloneEffectPresetState): void {}

    /** Runs on every state notification, after the generic bar re-rendered. */
    protected _afterStateRender(_state: StandaloneEffectPresetState): void {}

    // ── DOM cache ────────────────────────────────────────

    protected _cacheElements(root: ShadowRoot): Record<string, HTMLElement> {
    ...
    private _attachEventListeners(root: ShadowRoot) {
        ...
        root.addEventListener("click", (e) => {
            ...
            if (actionEl) {
                ...
                this._afterActionActivated(actionEl);
                return;
            }
    ...
    protected _handleAction(action: string) {
        switch (action) {
    ...
    private _handleCtxAction(action: string) {
        ...
        switch (action) {
            ...
            case "overwrite": {
                ...
                this._handleMutationResult(result);
                this._closeCtxMenu();
                break;
            }
            case "copy": {
    ...
    private _doRevert() {
        ...
        if (result) this._handleMutationResult(result, "Reverted to saved values");
    }
    ...
    protected _handleSoundReplacementResult(
    ...
    private _openSoundReplacementDialog() {
        ...
        this._showDialog("sound-replacement-dialog");
    }

    protected _cancelSoundReplacement() {
    ...
    protected _addDocumentListeners() {
