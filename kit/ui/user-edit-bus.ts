...
/** A command's reporting context, captured before asynchronous acceptance. */
export type UserEditReporter = {
    readonly parameterEdit: (edit: UserParameterEdit) => void;
    readonly gestureStart: () => void;
    readonly gestureEnd: () => void;
};
...
export function captureUserEditReporter(): UserEditReporter {
    const suppressed = programmaticWriteDepth > 0;
    return Object.freeze({
        parameterEdit(edit: UserParameterEdit) {
            if (suppressed) return;
            for (const listener of listeners) listener.onParameterEdit?.(edit);
        },
        gestureStart() {
            if (suppressed) return;
            for (const listener of listeners) listener.onGestureStart?.();
        },
        gestureEnd() {
            if (suppressed) return;
            for (const listener of listeners) listener.onGestureEnd?.();
        },
    });
}
