...
fixes are ordinary commits in the private Cmajor/CHOC repositories selected by
`cmake/dependencies.lock.cmake`; resolver and fork behavior tests fail if those fixes disappear. Worker JavaScript errors report status without unloading an otherwise
