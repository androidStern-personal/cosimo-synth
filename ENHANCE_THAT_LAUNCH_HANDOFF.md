...
The September 5/6 evidence tables and roles below are historical. This status and the current REC entries supersede them.
...
Evidence starting points: source `docs/BK_HOST_INTEGRATION.md`; `/private/tmp/builder-kit-customer-acceptance.ch1sNY10/RESULTS.md`; `/private/tmp/builder-kit-host-candidate.sh870fvh/candidate-summary.json` and `terminal-handoff.json`; `/Users/winterfell/Library/Application Support/BuilderKitReleases/v0.1.2-20260904-86xvBV/live-verification.json`. The relevant sanitized customer and engineering evidence is preserved in the [launch index](docs/enhance-that-launch/README.md). Do not copy private delivery URLs, tokens or credentials into Git or public assets. Temporary local paths are not the eventual public handoff.

## 3. Ownership and execution rules

- **Sidebar organization:** immediately place every top-level worker/reviewer Bob creates in **Bob's tasks** (section `8c41351e-9e69-4eb3-ad4d-d4286975d7dd`), and keep existing still-needed Bob-owned tasks there. Leave Bob's coordinator, Woods, independently started tasks, resident-data research and its workers in place; do not move projects. Archive completed or superseded workers only after safe handoff and when no unfinished repair, review or integration needs the task; idle or running work is not complete. Archiving never deletes transcripts, files, branches, worktrees or artifacts. No per-task approval or new tracker is required.
- **Andrew action requests:** send every actionable request promptly to Woods in task `01a03885-0e68-7411-8b3f-f69ae41b6d89`, stating exactly what Andrew must do, the app/window or direct location, why it is needed, and what work is blocked. Verify current state; send a short resolution update when completed. A worker message or foregrounded window alone is insufficient. Consolidate related requests, do not repeat already-approved actions, and do not navigate Andrew into another task without his explicit request. Woods alone relays requests in his chat and texts Andrew; workers must not independently text or create duplicate reminders. Never include credentials, passwords or capability URLs in texts.
...
- **Bob:** the engineering manager, not an implementer or product decision-maker. Existing coordinator: **Bob — Build reliability and AGENTS cleanup**, thread `01a06ded-e922-7b60-83a7-44332a0e3fee`, **GPT-6 Astra/HIGH, standard speed, Full Access**. He sequences assignments, manages dependencies/owners, delegates independent review and repair loops, arranges reviewed integration and release verification, and maintains the queue. He does not write product code, make assets, run the engineering test/build work himself or quietly fix a worker's implementation. Assign those execution activities, including integration operations, to appropriate top-level tasks under his sole coordination. Queue/handoff administration and reading status/review evidence are his own work. Reuse this owner; no competing Bob.
- **Workers:** create real, visible, top-level Codex tasks through the standard **Create Task tool/API**, not hidden subagents, collaboration workers, raw app-server thread creation, CLI subprocess agents or private scripts as substitutes. Launch workers and reviewers use **GPT-6 Astra, HIGH by default, standard speed (Fast off), Full Access** for ordinary launch work. Use **XHIGH** only for genuinely difficult product work; no **Max** launch work. The separate data/state researcher remains **Max**. This supersedes older launch-worker effort directions. Bob assigns concrete scopes and shared resources to existing owners. Use ordinary bounded commands; one-off runtime snapshots, no-op probes and execution-review machinery are retired from the active workflow, with original outputs preserved.
...
#### Approved correction — normal customer checkout and whole-store copy (September 6)

Andrew explicitly approved this correction in Woods task `01a03885-0e68-7411-8b3f-f69ae41b6d89`. It supersedes any L4 instructions or tests that normalize the current developer-facing payment-status screen as the ordinary buying journey. This is execution work, not another proposal awaiting product decisions.

**Required experience:** Buy → Polar's hosted checkout collects customer details and payment → automatic confirmation on our site → the correct purchase access and downloads. Customers must not operate our payment integration. Recovery belongs in a separate, clearly named account/help link, not an extra mandatory checkout step. Use a small, separate test-mode indicator in the private preview; the remaining experience and copy must be suitable for an ordinary buyer.

**Procedure and acceptance:**

1. Check Polar's current official hosted-checkout documentation and working example, then use that established integration as the baseline. One purchase button takes the buyer to Polar; remove the preliminary email form and intermediate continue/status screen unless a concrete, unavoidable integration requirement is demonstrated. Keep checkout creation, duplicate-payment prevention and confirmation inside the system. A genuine delayed confirmation may show a simple automatic waiting state; do not make manual status checking the normal journey or pretend payment succeeded before verification.
2. Review the entire product page, purchase journey, confirmation, download/access, recovery and customer emails as a buyer. Clearly explain the free plugin versus paid Builder Kit, approved price, what the buyer receives, and the next action. Preserve the settled offer and truthful rights/compatibility/support disclosures. Remove engineering jargon, test instructions, deployment/qualification commentary and explanations of internal product decisions from customer copy. Do not disguise unfinished functionality with disclaimers. Final visual branding is still deferred; do not generate new media or invent new product promises.
3. Trace where the current flow and copy were introduced: distinguish written requirements from implementation choices and correct any conflicting instructions at their source. Inspect the existing payment verification, purchase records, lifetime access, delivery, recovery and duplicate-payment protection before deciding what to retain. Replace the mistaken customer flow and remove obsolete code rather than adding another conditional screen or relabeling its buttons. Retain useful server-side behavior and relevant focused tests; do not rewrite working commerce wholesale or build new orchestration/proxy/test-management machinery. Exceptional recovery paths must not determine what every buyer sees.
4. Exercise the connected private preview as an actual customer: product page → one purchase action → real Polar sandbox checkout → automatic confirmation → receipt/access email → correct download. Also check cancellation, delayed confirmation and recovery of an existing purchase; keep the existing offer, access and consent safeguards. Check the whole journey and copy at mobile and desktop sizes, not just whether endpoints return success. Use ordinary commands, focused integration checks and direct browser interaction. No inventory system, tests about tests, elaborate evidence framework or new execution supervisor. Report any genuinely blocked proof honestly; successful isolated components do not count as a passed buying journey.
5. Deliver the reduced, reviewed implementation, working private-preview link, exact source changes and a concise statement of what passed and what remains unproved. Bob coordinates the existing L4 store owner (`01a07068-6002-7091-95c9-a9424541a669`) and appropriate independent review; Bob does not implement. Woods reviews the changed code and complete customer journey before presenting it as ready to Andrew. Do not make Andrew teach the worker conventional checkout behavior or repeatedly navigate unfinished intermediate screens.

**Boundaries:** use the existing Astra/standard-speed worker settings and existing launch owners. Preserve unrelated site/products, customer records, secrets and current artifacts. Existing private-preview/sandbox authority remains; no public launch, real charges/refunds, marketing sends, broader credentials or unapproved email recipients. Surface genuinely required user actions through Woods under section 3; do not repeat previously granted approvals. Bob records this correction under the existing L4/BK-50–52 queue and keeps the authoritative brief available to its worker and reviewer without creating a competing process.
