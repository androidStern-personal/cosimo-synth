...
## September 9 — active recovery handoff

Andrew approved the seven-step recovery and customer-runthrough plan in task `01a08549-db5f-7dc1-ad7b-bb91acf0802b`; that task now owns integration, release coordination, website delivery and readiness. The current user-provided `AGENTS.md` governs isolated implementation (SOL/max) and independent parallel work. Use the REC-1–4 queue at the top of `TODOS.txt`. The older Bob/Woods staffing and top-level worker creation rules below are historical for this recovery. No task/worktree deletion or archival is planned.

**Maintained product source:** `/Users/winterfell/src/cosimo-synth`, GitHub `androidStern-personal/cosimo-synth`, branch `master`. Recovery integration worktree: `/Users/winterfell/.codex/worktrees/builder-kit-recovery-integration/cosimo-synth`. The private Enhance That source `f30f358f` has been reviewed and composed with current planning in the recovery integration worktree. Review found and repaired stale shared dependency contracts/notices and an unsafe option for packaging an existing build. Final qualification and integration remain pending. The complete [roadmap](ENHANCER_LITE_BUILDER_KIT_ROADMAP.md) is preserved alongside this handoff; `TODOS.txt` tracks active assignments rather than replacing the roadmap.

**Maintained website:** `/Users/winterfell/Documents/Codex/2026-09-09/song-machines-builder-kit-checkout`. Saved composition `ed24e0b` includes the new landing page plus guest commerce. Public `song-machines.com` remains a waitlist; the protected Vercel sandbox is the checkout runthrough. These become two deployment modes of this maintained website, with original prototype/public-release folders retained as evidence. Website onboarding worktree: `/Users/winterfell/.codex/worktrees/builder-kit-website-onboarding/song-machines-website`.

**Current downloadable candidate before recovery:** Builder Kit 0.1.3 customer `1bf9e893` from source `9928d599`, free AU+VST3 package from `f30f358f`. They are explicitly uploaded private artifacts, not automatic pulls from a local checkout or GitHub main. The ordinary release mirror remains 0.1.2. The replacement onboarding candidate must be traceable to its exact reviewed source and tools; do not overwrite an existing version's immutable artifacts. Both the replacement kit and free package use candidate version 0.1.4. The free package is rebuilt because its notices must match the selected dependencies. Verified development-tool archive bytes may be reused under the new immutable version namespace because their build inputs are unchanged.

**Remaining customer work:** visible JUCE acknowledgment; Apple Silicon/macOS15+ and development prerequisites before payment; agent-guided Documents destination and checks; final-candidate update/recovery; connected private checkout/download/setup/build/host runthrough. No unsupported Codex/Claude Code launch claim. Preserve the September 6 guest-store contract: free ungated plugin, paid editable kit, no account system, durable payment/delivery/email/recovery, no new-purchase controls on paid confirmation, case-by-case refunds and bounded support.

Existing private preview/Polar sandbox/test receipt permissions remain available. Public paid launch, real charges/refunds, marketing sends, personal agreements and subjective musical acceptance remain separate. Seven-day beta and Andrew's runthrough are not represented as automated passes.

The September 5/6 evidence tables and roles below are historical; current facts above and current REC entries supersede them.
