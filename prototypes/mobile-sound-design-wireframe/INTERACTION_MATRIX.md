...
| Mapping chip | Quick-adjust or focus a relationship | Vertical drag adjusts amount; tap opens that relationship in the contextual editor | Opening focus replaces only the already-fixed workspace; no shell region moves |
| Contextual mapping editor | Edit relationship deeply | Collapse the module to a live target strip; keep its base value editable; give the source graphic focus; adjust amount, polarity, reducer when required, remove mapping, or open the source | Target strip, source editor, and relationship settings have stable internal slots |
