...
| Module graphic | Shape the focused module | Direct manipulation edits the module's named X/Y parameters and updates the transient HUD | Compound controls remain in a reserved overlay lane. The graphic carries no outline frame (2026-07-18 — real module graphics land here later and arrive frameless) |
| Parameter matrix | Select and edit | Tap selects; horizontal drag edits the effective base layer; vertical drag edits the focused mapping's amount. An articulation route-amount override marks the cell like a base override: articulation icon in the heading, Y value in the articulation color (2026-07-18). While worn, edits to global effect targets stay patch-base and the HUD appends “GLOBAL — EDITS PATCH BASE” | Every cell has stable tracks, source marks, and value lanes. Selection is corner ticks (the shared focus grammar), never a full outline — full outlines are reserved for drag-drop feedback (2026-07-18) |
...
| Articulation strip (transport) | Choose audition articulation / enter the edit layer | The transport articulation control renders identically on every workspace, including Articulations (amended 2026-07-18, reversing the same-day exclusion — one strip, one geometry, everywhere); selecting a non-Default articulation exposes an EDIT latch beside it; the latch wears that articulation | Transport row geometry is fixed; the latch occupies reserved space within the articulation group |
...
| Articulations workspace · trigger lane | Assign playback triggers | One mode visible at a time (Key/Vel/Chain). Key: a ~1.5-octave piano is display only, with octave paddles; a separate labeled drag handle (plus ∓1 nudges) moves the selected articulation's keyswitch so the finger never occludes placement, and movement clamps flush against neighbors with a haptic at first contact. Vel/Chain: partition strips with tap-to-select segments and scrubbed MIN/MAX values; ranges may NEVER overlap — a dragged bound stops flush at the neighboring range's edge with the same contact haptic, so growing into a neighbor requires shrinking the neighbor first. Play-to-set appears only when MIDI input is present and is never required | The lane is display plus chunky controls; no manipulation target may fall below the touch minimum. Piano markers carry no text (color is the identity; the handle names it), and no HUD readout may cover the lane — the handle and bound fields are their own value homes |
