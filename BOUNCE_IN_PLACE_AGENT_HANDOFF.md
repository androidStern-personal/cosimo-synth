...
- You are already on the right branch: **`codex/bounce-in-place`**, forked
  from `origin/claude/effects-lane-m1` at `367922d`. Work here and push here.
  If `origin/claude/effects-lane-m1` moves and you need its newer work, merge
  it forward (never rebase).
...
### Reference material (read it first)

Two prior feasibility studies and a sizing probe are already committed on
this branch under `docs/reference/`:
`BOUNCE_IN_PLACE_FEASIBILITY.md`, `AUDIO_ASSET_OWNERSHIP_FEASIBILITY.md`,
and `PROBE_bounce_math.mjs`. Read the bounce study in full before writing
code.
