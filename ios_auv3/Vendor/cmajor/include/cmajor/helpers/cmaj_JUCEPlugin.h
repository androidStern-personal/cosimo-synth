...
protected:
    ...
    {
        Parameter (JUCEPluginBase& ownerToNotify, juce::String&& pID)
    ...
    void createParameterTree()
    {
        ...
        if constexpr (DerivedType::isPrecompiled || DerivedType::isFixedPatch)
        {
            struct ParameterTreeBuilder
            {
                ...
                {
                    auto newParam = std::make_unique<Parameter> (owner, param->properties.endpointID);
    ...
    void ensureNumParameters (size_t num)
    {
        ...
        {
            auto p = std::make_unique<Parameter> (*this, "P" + juce::String (parameters.size()));
...
class GeneratedPlugin  : public JUCEPluginBase<GeneratedPlugin<GeneratedPerformerClass>>
{
public:
    using super = JUCEPluginBase<GeneratedPlugin<GeneratedPerformerClass>>;
    ...
    bool prepareManifest (Patch::LoadParams& loadParams, const juce::ValueTree& newState) override
    {
        ...
        static constexpr std::string_view runtimePatchManifestFilename = "WavetableSynth.iOS.cmajorpatch";
        ...
        const auto getRuntimeResourceFile = [] (const std::string& path) -> juce::File
        {
           ...
           #if defined(COSIMO_ENABLE_LIVE_REPO_RESOURCES) && COSIMO_ENABLE_LIVE_REPO_RESOURCES && defined(COSIMO_REPO_ROOT_PATH)
            {
                const auto repoRoot = juce::File (juce::String (COSIMO_REPO_ROOT_PATH));

                if (repoRoot.isDirectory())
                {
                    const auto directRepoFile = repoRoot.getChildFile (relativePath);

                    if (directRepoFile.existsAsFile())
                        return directRepoFile;

                    const auto vendoredWebFile = repoRoot.getChildFile ("ios_auv3")
                                                          .getChildFile ("Vendor")
                                                          .getChildFile ("cmajor")
                                                          .getChildFile ("web")
                                                          .getChildFile (relativePath);

                    if (vendoredWebFile.existsAsFile())
                        return vendoredWebFile;
                }
            }
           #endif
        ...
        loadParams.manifest.initialiseWithVirtualFile (std::string (runtimePatchManifestFilename),
    ...
    using PerformerClass = GeneratedPerformerClass;
