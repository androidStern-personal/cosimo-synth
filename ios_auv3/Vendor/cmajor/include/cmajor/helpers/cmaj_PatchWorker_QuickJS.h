...
inline void enableQuickJSPatchWorker (Patch& p)
{
    struct Worker : Patch::WorkerContext
    {
                         ...
                         std::function<void(const std::string&)> reportError) override
        {
            ...
            choc::javascript::Context::ReadModuleContentFn resolveModule = [&p, workerType = workerType] (std::string_view path) -> std::optional<std::string>
            {
                ...
                if (auto manifest = p.getManifest())
                {
                    const auto appendCandidate = [] (std::vector<std::string>& candidates, std::string candidate)
                    {
                        candidate = normaliseManifestResourcePath (candidate);
                    ...
                    appendCandidate (candidates, std::string (path));

                    const auto workerModulePath = normaliseManifestResourcePath (
                        workerType == "sourceTransformer" ? manifest->sourceTransformer
                                                          : manifest->patchWorker);

                    if (! workerModulePath.empty())
                    {
                        const auto workerDirectory = std::filesystem::path (workerModulePath).parent_path();

                        if (! workerDirectory.empty())
                            appendCandidate (candidates, (workerDirectory / std::filesystem::path (path)).generic_string());
                    }

                    for (const auto& candidate : candidates)
                        if (auto module = readJavascriptResource (candidate, manifest))
                            return module;
                }
