...
inline int decodeManifestResourceHexDigit (char c)
{
    if (c >= '0' && c <= '9')
        return c - '0';
...
inline std::string normaliseManifestResourcePath (std::string_view rawPath)
{
    std::string pathText (rawPath);
    ...
    {
        const auto c = pathText[index];

        if (c == '%' && index + 2 < pathText.size())
        {
            const auto hi = decodeManifestResourceHexDigit (pathText[index + 1]);
            const auto lo = decodeManifestResourceHexDigit (pathText[index + 2]);
    ...
    auto relativePath = std::filesystem::path (decoded).lexically_normal().relative_path().generic_string();
...
inline std::optional<std::string> PatchManifest::readFileContent (const std::string& file) const
{
    try
    {
        if (auto stream = createFileReader (normaliseManifestResourcePath (file)))
        {
                                                           ...
                                                           const choc::value::ValueView& annotation)
{
    ...
    if (auto reader = manifest.createFileReader (normaliseManifestResourcePath (path)))
    {
