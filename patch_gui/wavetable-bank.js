// Generated from ui/shared/wavetable-bank.ts by node ui/build.mjs. Do not edit this file directly.
import { asResourceClient, } from "./resource-client.js";
...
function extractSourceFrames(samples, { expectedFrameCount, samplesPerFrame = DEFAULT_SAMPLES_PER_FRAME, } = {}) {
    assert(samples.length % samplesPerFrame === 0, `Source wavetable files must contain a whole number of ${samplesPerFrame}-sample frames`);
...
export async function loadFactoryBankCatalogFromPatch(patchConnection, { catalogPath = DEFAULT_FACTORY_BANK_CATALOG_PATH, } = {}) {
    return loadFactoryBankCatalog(patchConnection, { catalogPath });
}
export async function loadFactoryBankCatalog(resourceClientInput, { catalogPath = DEFAULT_FACTORY_BANK_CATALOG_PATH, } = {}) {
...
export async function loadFactoryBankFramesFromPatch(patchConnection, { catalogPath = DEFAULT_FACTORY_BANK_CATALOG_PATH, tableIndex = 0, samplesPerFrame = DEFAULT_SAMPLES_PER_FRAME, } = {}) {
    return loadFactoryBankFrames(patchConnection, { catalogPath, tableIndex, samplesPerFrame });
}
export async function loadFactoryBankFrames(resourceClientInput, { catalogPath = DEFAULT_FACTORY_BANK_CATALOG_PATH, tableIndex = 0, samplesPerFrame = DEFAULT_SAMPLES_PER_FRAME, } = {}) {
    const resourceClient = asResourceClient(resourceClientInput);
