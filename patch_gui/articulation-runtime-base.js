// Generated from ui/shared/articulation-runtime-base.ts by node ui/build.mjs. Do not edit this file directly.
 ...
 */
export function getArticulationModulationDependencyToken(state) {
...
export function getArticulationPatchValuesDependencyToken(patchValues) {
    return JSON.stringify(allTargetDescriptors().flatMap((descriptor) => {
        ...
        return [[
                parameterID,
                descriptor.binding.toEngine(clampNormalizedValue(patchValues[descriptor.targetId] ?? descriptor.initialValue)),
            ]];
    }));
}
export function createDefaultUiPatchValues() {
    return Object.fromEntries(allTargetDescriptors().map((descriptor) => [descriptor.targetId, descriptor.initialValue]));
}
export function deserializeUiPatchValues(value) {
    let document = value;
...
export function buildArticulationPatchVoiceBase(modulationState, patchValues) {
    ...
    for (const descriptor of allTargetDescriptors()) {
        ...
        parameters[parameterID] = descriptor.binding.toEngine(clampNormalizedValue(patchValues[descriptor.targetId] ?? descriptor.initialValue));
    }
    const runtimeRoutes = modulationState.routes.flatMap((route) => {
        const cellIndex = getModulationArticulationCellIndex(route);
        return cellIndex === null ? [] : [{ route, cellIndex }];
    });
    const routeCells = Object.create(null);
