...
async function initialisePatch() {
    ...
    const createViewIfNeeded = async () => {
        ...
        if (!view) {
            globalThis.setStatusMessage("No view available");
            return;
        }
    ...
    const statusListener = async (status) => {
        if (status?.error) {
            const manifestName = status?.manifest?.name;
            const prefix = manifestName ? `Error building '${manifestName}':` : "Error:";
            globalThis.setStatusMessage(`${prefix}\n\n${status.error.toString()}`);
            return;
        }
    ...
    patchConnection.sendEventOrValue?.("runtimeSyncRequest", 1);
}
