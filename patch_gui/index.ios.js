...
function clampFilterCutoffHz(value) {
  return clamp$9(Number(value) || 0, FILTER_CUTOFF_MIN_HZ, FILTER_CUTOFF_MAX_HZ);
}
...
function clampWarpMode(value) {
  return clamp$9(Math.round(Number(value) || 0), WARP_MODE_OFF$1, WARP_MODE_MIRROR$1);
}
...
function pointToMsegEditorCoordinates(point, width, height, options = {}) {
  ...
  const normalizedX = clamp01$1(Number(point?.x));
  const normalizedY = clamp01$1(Number(point?.y));
...
function msegEditorCoordinatesToPoint(editorX, editorY, width, height, options = {}) {
  ...
  if (orientation === "vertical") {
    return {
      x: clamp01$1((Number(editorY) - metrics.plotTop) / metrics.plotHeight),
      y: clamp01$1((Number(editorX) - metrics.plotLeft) / metrics.plotWidth)
    };
  }
  return {
    x: clamp01$1((Number(editorX) - metrics.plotLeft) / metrics.plotWidth),
    y: clamp01$1(1 - (Number(editorY) - metrics.plotTop) / metrics.plotHeight)
  };
}
...
function evaluateMsegSegmentPoint(from, to, t) {
  const clampedT = clamp01$1(t);
  const curvedT = clamp01$1(powerScale(clampedT, from.curvePower));
  return {
...
function deriveMsegSegmentCurvePower(shape, segmentIndex, x, y) {
  ...
  const localX = clamp$8(clamp01$1(Number(x)), from.x, to.x);
...
function addMsegPoint(shape, x, y) {
  ...
  const nextPoint = {
    x: clamp01$1(Number(x)),
    y: clamp01$1(Number(y)),
    curvePower: 0
  };
...
function moveMsegPoint(shape, pointIndex, x, y) {
  ...
  moved.y = clamp01$1(Number(y));
  if (pointIndex === 0) {
    moved.x = 0;
  } else if (pointIndex === points.length - 1) {
    moved.x = 1;
  } else {
    moved.x = clamp$8(clamp01$1(Number(x)), previousX, nextX);
  }
...
function clampEnvSeconds(value, fallback) {
  ...
  return clamp$6(Number.isFinite(numeric) ? numeric : fallback, ENV_MIN_SECONDS, ENV_MAX_SECONDS);
}
...
function getModulationAmountDepth(targetKind, amount) {
  ...
  return clamp$6(Math.abs(clampedAmount) / limit, 0, 1);
}
function composeModulationAmount(targetKind, depth) {
  const limits = getRouteAmountLimit(targetKind);
  const clampedDepth = clamp$6(Number.isFinite(depth) ? depth : 0, 0, 1);
...
function getModulationAmountSliderPosition(targetKind, amount) {
  ...
  if (clampedAmount < 0) {
    ...
    return clamp$6(0.5 * (1 - Math.abs(clampedAmount) / Math.abs(limits.min)), 0, 0.5);
  }
  if (Math.abs(limits.max) <= 1e-9) {
    return 0.5;
  }
  return clamp$6(0.5 + 0.5 * (clampedAmount / limits.max), 0.5, 1);
}
...
class ModulationRuntimeBridge {
  ...
  requestBootState() {
    if (typeof this.patchConnection.requestFullStoredState === "function") {
      this.patchConnection.requestFullStoredState((storedState) => {
        this.applyStoredState(readFullStoredStateValue$1(storedState, MODULATION_STATE_KEY));
      });
      return;
    }
  ...
  getMsegSlotController(slotIndex) {
    return this.slotControllers[clamp$6(Math.round(slotIndex), 0, MODULATION_MSEG_SLOT_COUNT - 1)];
  }
  getMsegSlotEditShapeIndex(slotIndex) {
    return this.msegSlotEditShapeIndexes[clamp$6(Math.round(slotIndex), 0, MODULATION_MSEG_SLOT_COUNT - 1)];
  }
  setMsegSlotEditShapeIndex(slotIndex, shapeIndex) {
    const normalizedSlotIndex = clamp$6(Math.round(slotIndex), 0, MODULATION_MSEG_SLOT_COUNT - 1);
  ...
  setMsegSlotShape(slotIndex, shapeIndex, nextShape) {
    ...
    this.updateState((previousState) => {
      const nextMsegSlots = previousState.msegSlots.map((slot, index) => index === slotIndex ? Math.round(Number(shapeIndex)) === 1 ? { ...slot, shapeB: normalizedShape } : { ...slot, shapeA: normalizedShape } : slot);
      return {
        ...previousState,
...
function buildMsegMorphCurvePath(shapeAPoints, shapeBPoints, morphValue, width, height, options = {}) {
  ...
  try {
    ...
    const morph = clamp$5(Number(morphValue) || 0, 0, 1);
    const metrics = createMsegEditorMetrics(width, height, {
      pointRadius: options.pointRadius,
      horizontalPadding: options.horizontalPadding ?? MSEG_EDITOR_HORIZONTAL_PADDING_PX,
      verticalPadding: options.verticalPadding ?? MSEG_EDITOR_VERTICAL_PADDING_PX
...
function MsegPreview({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "svg",
    {
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("defs", { children: progressClipRect ? /* @__PURE__ */ jsxRuntimeExports.jsx("clipPath", { id: clipPathId, children: /* @__PURE__ */ jsxRuntimeExports.jsx(
          "rect",
          {
...
function EditableMsegSurface({
  ...
  const { curvePath, fillPath, referenceCurvePath, referenceFillPath, morphCurvePath, highlightedSegmentPath, metrics } = reactExports.useMemo(() => {
    const basePaths = buildMsegSurfacePaths(points, size.width, size.height, {
      orientation
    });
    const referencePaths = referencePoints ? buildMsegSurfacePaths(referencePoints, size.width, size.height, { orientation }) : null;
    const nextMorphCurvePath = showMorphCurve ? buildMsegMorphCurvePath(morphShapeAPoints, morphShapeBPoints, morphValue, size.width, size.height, { orientation }) : "";
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "svg",
    {
      ...
      children: [
        ...
        referenceFillPath ? /* @__PURE__ */ jsxRuntimeExports.jsx(
          "path",
          {
        ...
        referenceCurvePath ? /* @__PURE__ */ jsxRuntimeExports.jsx(
          "path",
          {
...
function findBoundPeak(mins, maxs) {
...
function normalizeDistortionScopeMessage(message) {
  ...
  return {
    ...
    dominantChannel: clamp$4(Math.round(coerceFiniteNumber(record.dominantChannel) ?? 0), 0, 1),
...
function normalizeDistortionHistoryMessage(message) {
  const payload = message && typeof message === "object" && "event" in message && message.event ? message.event : message;
  if (!payload || typeof payload !== "object") {
    return null;
  }
  ...
  const horizonMs = Math.max(
    0,
    coerceFiniteNumber(record.horizonMs) ?? (coerceFiniteNumber(record.binDurationMs) ?? 0) * binCount
  );
  const binDurationMs = Math.max(
    0,
    coerceFiniteNumber(record.binDurationMs) ?? (horizonMs > 0 ? horizonMs / Math.max(1, binCount) : 0)
  );
  return {
    sampleRateHz: Math.max(1, coerceFiniteNumber(record.sampleRateHz) ?? 44100),
    horizonMs,
...
function shapeDistortionSample(inputSample, knee) {
  const clampedKnee = clamp$4(Number(knee) || 0, 0, 1);
...
function buildDistortionHistoryBins(frame) {
  ...
  const safeValidBinCount = clamp$4(frame.validBinCount, 0, activeBinCount);
...
function buildDistortionTransferOccupancy({
  ...
  const rawPoints = Array.from({ length: safeBinCount }, (_, index) => {
    ...
    return {
      ...
      clipped: clamp$4(smoothedClipped[index] ?? 0, 0, 1)
    };
  });
  const segments = [];
  let currentSegment = [];
...
function DistortionVisualizer({
  ...
  reactExports.useEffect(() => {
    ...
    setDisplayState((previousState) => advanceDistortionDisplayState(previousState, transferFrame, performance.now()));
  }, [transferFrame]);
  ...
  const historyBins = reactExports.useMemo(
  ...
  const transferOccupancyPaths = reactExports.useMemo(() => transferOccupancy.segments.map((segment) => {
    ...
    return {
      occupancyPath,
      clippedPath,
      occupancyOpacity: clamp$3(0.14 + peakDensity * 0.34, 0.14, 0.48),
      clippedOpacity: clamp$3(peakRemoved * 0.62 + peakClipped * 0.24, 0, 0.72)
    };
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "svg",
        {
          ...
          children: [
            ...
            historyColumns.map((column, index) => /* @__PURE__ */ jsxRuntimeExports.jsxs("g", { children: [
              column.valid ? /* @__PURE__ */ jsxRuntimeExports.jsx(
                "rect",
                {
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "overflow-hidden rounded-[24px] border border-white/8 bg-[linear-gradient(180deg,rgba(2,6,18,0.95),rgba(1,3,9,1))] shadow-[inset_0_1px_0_rgba(255,255,255,0.06)]", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "svg",
      {
        ...
        children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("text", { x: HISTORY_PLOT.left + 14, y: HISTORY_PLOT.top + 22, fill: "rgba(226,232,240,0.58)", fontSize: "11", letterSpacing: "0.2em", children: "OUTPUT + DELTA" }),
          [ceilingYTransferTop, zeroYTransfer, ceilingYTransferBottom].map((yValue, index) => /* @__PURE__ */ jsxRuntimeExports.jsx(
            "line",
            {
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("text", { x: HISTORY_PLOT.left + HISTORY_PLOT.width - 10, y: HISTORY_PLOT.top + 24, fill: "rgba(226,232,240,0.54)", fontSize: "11", textAnchor: "end", children: historyFrame ? `${(historyFrame.horizonMs / 1e3).toFixed(1)}s overview` : "waiting" })
        ]
      }
    ) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("pre", { "data-role": "distortion-graph-debug", className: "hidden", children: JSON.stringify(debugState) })
...
const FULL_SPAN_LOOP_EPSILON = 1e-6;
...
function coercePositions(value) {
...
function normalizeEffectiveMsegStateMessage(message) {
  const payload = message?.event ?? message;
  if (!payload || typeof payload !== "object") {
    return null;
  }
  const positions = coercePositions(payload.positions);
  if (!positions) {
    return null;
  }
  const rawGeneration = Number(payload.voiceGeneration);
...
function selectObservedEffectiveMsegState(currentState, message) {
  const previousState = currentState && typeof currentState === "object" ? {
    voiceGeneration: Number.isFinite(Number(currentState.voiceGeneration)) ? Math.trunc(Number(currentState.voiceGeneration)) : -1,
    hasActive: Boolean(currentState.hasActive),
    positions: coercePositions(currentState.positions) ?? [0, 0, 0]
  } : {
    voiceGeneration: -1,
    hasActive: false,
    positions: [0, 0, 0]
...
function hasFullSpanLoop(playback) {
  const loop = playback?.loop;
  if (!loop) {
    return false;
  }
  return loop.startX <= FULL_SPAN_LOOP_EPSILON && loop.endX >= 1 - FULL_SPAN_LOOP_EPSILON;
}
function resolveMsegPreviewPlayheadState({
  observedState,
  playback,
  slotIndex
}) {
  const clampedSlotIndex = clampSlotIndex(slotIndex);
  const voiceGeneration = Number.isFinite(Number(observedState?.voiceGeneration)) ? Math.trunc(Number(observedState?.voiceGeneration)) : -1;
  ...
  const progress = clamp01$1(observedState.positions[clampedSlotIndex]);
  const shouldFillProgress = playback ? !playback.loop || hasFullSpanLoop(playback) : false;
  return {
    voiceGeneration,
    hasActive: true,
    progress,
...
const IOSMsegLauncher = reactExports.memo(function IOSMsegLauncher2({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mseg-shell", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mseg-launcher", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "grid", gridTemplateColumns: "auto minmax(0, 1fr)", gap: "0.75rem", alignItems: "center", marginBottom: "0.75rem" }, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { display: "flex", gap: "0.35rem" }, children: [0, 1].map((shapeIndex) => /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          type: "button",
        ...
        `ios-mseg-shape-${shapeIndex}`
      )) }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "mseg-rate", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "mseg-depth-label", children: "Morph" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "input",
          {
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "start", gap: "0.75rem" }, children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { className: "mseg-route-title", children: "Driven Curve + Delta" })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "div",
            {
              ...
              children: [
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsx("div", { children: `Out ${outputPeak.toFixed(3)} • Delta ${removedPeak.toFixed(3)}` })
              ]
            }
          )
        ] }),
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.5rem" }, children: DISTORTION_MODE_OPTIONS.map((option) => {
            ...
            return /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "button",
              {
                ...
                children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { fontSize: "0.73rem", letterSpacing: "0.08em", textTransform: "uppercase" }, children: option.label }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { fontSize: "0.67rem", color: active ? "rgba(207,250,254,0.82)" : "rgba(203,213,225,0.62)" }, children: option.summary })
                ]
              },
              option.value
            );
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "grid", gap: "0.8rem" }, children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { style: { display: "grid", gap: "0.32rem" }, children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", justifyContent: "space-between", gap: "0.75rem" }, children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "mseg-depth-label", children: "Band HP" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "span",
                {
                  style: {
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { style: { display: "grid", gap: "0.32rem" }, children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", justifyContent: "space-between", gap: "0.75rem" }, children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "mseg-depth-label", children: "Band LP" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "span",
                {
                  style: {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "div",
          {
            ...
            children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { color: "rgba(251,191,202,0.78)" }, children: `${modeLabel} Readout` }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { color: "rgba(241,245,249,0.9)" }, children: `Input ${inputPeak.toFixed(3)}` }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { color: "rgba(165,243,252,0.9)" }, children: `Output ${outputPeak.toFixed(3)}` }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { color: "rgba(251,113,133,0.9)" }, children: `Delta ${removedPeak.toFixed(3)}` })
            ]
          }
        )
      ]
      ...
      children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mseg-modal-footer", children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mseg-modal-footer-actions", children: [
            msegState ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { display: "flex", gap: "0.35rem" }, children: [0, 1].map((shapeIndex) => /* @__PURE__ */ jsxRuntimeExports.jsx(
              "button",
              {
                className: "mseg-loop-button",
                type: "button",
              ...
              `ios-modal-mseg-shape-${shapeIndex}`
            )) }) : null,
