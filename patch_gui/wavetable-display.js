...
function createProjection(points, width, height, drawableInsets = {}) {
    ...
    const scale = Math.min((drawable.width - padding.left - padding.right) / spanX, (drawable.height - padding.top - padding.bottom) / spanY);
    return {
        width,
        height,
        drawable,
...
function createSurfaceBands(projectedFrames) {
    ...
    for (let frameIndex = 0; frameIndex < projectedFrames.length - 1; frameIndex += 1) {
        ...
        for (let sampleIndex = 0; sampleIndex < frontFrame.points.length - 1; sampleIndex += 1) {
            ...
            const surfaceNormal = normaliseVector(crossProduct(subtractPoints(objectQuad[1], objectQuad[0]), subtractPoints(objectQuad[3], objectQuad[0])));
            ...
            const ridgeAmount = clamp((Math.abs(frontFrame.samples[sampleIndex + 1] - frontFrame.samples[sampleIndex]) * 0.95) +
                (Math.abs(backFrame.samples[sampleIndex + 1] - backFrame.samples[sampleIndex]) * 0.95), 0, 1);
            bands.push({
...
function createSurfaceRibs(projectedFrames) {
    ...
    for (let ribIndex = 1; ribIndex < desiredRibCount - 1; ribIndex += 1) {
        selectedColumns.add(Math.round((ribIndex * (sampleCount - 1)) / (desiredRibCount - 1)));
    }
    return [...selectedColumns]
        ...
        const frontness = 1 - (projectedFrames.reduce((total, frame) => total + frame.depthNormalized, 0) /
...
function createInterpolatedSurfaceSlices(sourceFrames, camera, projection) {
    ...
    for (let sliceIndex = 0; sliceIndex < sliceCount; sliceIndex += 1) {
        ...
        const samples = buildInterpolatedFrame(sourceFrames[frameLo].samples, sourceFrames[frameHi].samples, frameT);
        const depth = getSceneDepthAt(framePosition, frameCount);
        const objectPoints = createObjectPoints(samples, depth);
        const points = objectPoints.map((point) => projectToScreen(projectWorldPoint(point, camera), projection));
        const averageDepth = points.reduce((total, point) => total + point.cameraDepth, 0) / Math.max(points.length, 1);
        slices.push({
...
function createCurrentSlice(staticScene, frameState) {
    ...
    const points = objectPoints.map((point) => projectToScreen(projectWorldPoint(point, staticScene.camera), staticScene.projection));
    const floorPoints = floorObjectPoints.map((point) => projectToScreen(projectWorldPoint(point, staticScene.camera), staticScene.projection));
...
export function buildWavetableStaticScene({ frames, width = 640, height = 320, pixelRatio = 1, drawableInsets = {}, }) {
    assertFrames(frames);
...
export function buildWavetableRenderModel({ frames = null, position = 0, warpMode = 0, warpAmount = 0, width = 640, height = 320, pixelRatio = 1, drawableInsets = {}, staticScene = null, }) {
...
export class CanvasWavetableDisplay {
    ...
    staticKey;
    pendingRenderHandle;
    constructor(canvas, { theme = DEFAULT_WAVETABLE_THEME, requestAnimationFrame = requestNextAnimationFrame, cancelAnimationFrame = cancelNextAnimationFrame, paintBackground = true, showSliceCaption = true, } = {}) {
        this.canvas = canvas;
    ...
    setFrames(frames) {
        assertFrames(frames);
        this.frames = frames.map((frame) => frame instanceof Float32Array ? frame.slice() : Float32Array.from(frame));
    ...
    setDrawableInsets(insets = {}) {
        ...
        if (nextInsets.top === this.drawableInsets.top &&
