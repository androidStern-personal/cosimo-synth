...
export function clampMsegRateSeconds(value) {
    ...
    return clamp(Number.isFinite(numericValue) ? numericValue : 1.0, MSEG_RATE_MIN_SECONDS, MSEG_RATE_MAX_SECONDS);
}
export function createMsegEditorMetrics(width, height, { pointRadius = MSEG_POINT_RADIUS_PX, horizontalPadding = MSEG_EDITOR_HORIZONTAL_PADDING_PX, verticalPadding = MSEG_EDITOR_VERTICAL_PADDING_PX, } = {}) {
...
export function normalizeMsegPlayback(playback = createDefaultMsegPlayback()) {
    ...
    const noteOffPolicy = MSEG_NOTE_OFF_POLICY_VALUES.has(noteOffPolicyCandidate)
...
function distanceSquaredToLineSegment(targetX, targetY, fromX, fromY, toX, toY) {
    ...
    const projection = clamp((((targetX - fromX) * deltaX) + ((targetY - fromY) * deltaY)) / segmentLengthSquared, 0, 1);
...
export function sampleMsegSegmentEditorPolyline(shape, segmentIndex, width, height, editorOptions = {}) {
    ...
    const appendAdaptiveSamples = (startT, endT, startPointCoordinates, endPointCoordinates, depth) => {
        ...
        const errorSquared = distanceSquaredToLineSegment(midpointCoordinates.x, midpointCoordinates.y, startPointCoordinates.x, startPointCoordinates.y, endPointCoordinates.x, endPointCoordinates.y);
...
export function sampleMsegEditorPolyline(shape, width, height, editorOptions = {}) {
    const normalizedShape = normalizeMsegShape(shape);
    const polyline = [];
    for (let segmentIndex = 0; segmentIndex < normalizedShape.points.length - 1; segmentIndex += 1) {
        const segmentPolyline = sampleMsegSegmentEditorPolyline(normalizedShape, segmentIndex, width, height, editorOptions);
...
export function sampleRenderedMsegBuffer(paddedBuffer, x) {
    ...
    return catmullRom(paddedBuffer[sampleIndex], paddedBuffer[sampleIndex + 1], paddedBuffer[sampleIndex + 2], paddedBuffer[sampleIndex + 3], fractional);
}
export function findMsegPointHitIndex(shape, editorX, editorY, width, height, hitRadius = MSEG_POINT_HIT_RADIUS_PX, editorOptions = {}) {
...
export function findMsegSegmentHitIndex(shape, editorX, editorY, width, height, hitRadius = MSEG_SEGMENT_HIT_RADIUS_PX, editorOptions = {}) {
    const normalizedShape = normalizeMsegShape(shape);
    const targetX = Number(editorX);
    const targetY = Number(editorY);
    if (!Number.isFinite(targetX) || !Number.isFinite(targetY)) {
    ...
    let closestDistanceSquared = safeHitRadius * safeHitRadius;
    for (let segmentIndex = 0; segmentIndex < normalizedShape.points.length - 1; segmentIndex += 1) {
        const polyline = sampleMsegSegmentEditorPolyline(normalizedShape, segmentIndex, width, height, editorOptions);
        for (let pointIndex = 0; pointIndex < polyline.length - 1; pointIndex += 1) {
            ...
            const distanceSquared = distanceSquaredToLineSegment(targetX, targetY, from.x, from.y, to.x, to.y);
            if (distanceSquared <= closestDistanceSquared) {
...
export function deriveMsegSegmentCurvePower(shape, segmentIndex, x, y) {
    const normalizedShape = normalizeMsegShape(shape);
    if (!Number.isInteger(segmentIndex) || segmentIndex < 0 || segmentIndex >= normalizedShape.points.length - 1) {
...
export function toMsegPlaybackConfigEvent(playback) {
    ...
    return {
        ...
        noteOffPolicy: normalizedPlayback.noteOffPolicy === "immediate"
...
export function setMsegSegmentCurvePower(shape, segmentIndex, curvePower) {
    const normalizedShape = normalizeMsegShape(shape);
    if (!Number.isInteger(segmentIndex) || segmentIndex < 0 || segmentIndex >= normalizedShape.points.length - 1) {
        throw new Error("segmentIndex must address a segment inside the shape");
    }
