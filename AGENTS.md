...
- Before its final handoff, every implementation task must use the `decision-provenance` skill to identify the implementation decisions Andrew is most likely to object to or find fault with, including the chosen behavior, the rejected alternative, and the evidence or tradeoff. The integration coordinator must independently assess the severity of each surfaced decision against current product authority and the actual source. A materially bad decision is a failed review: return a focused repair to the owning task and do not run final broad/native gates or integrate until it is corrected. Preserve the objection audit and the coordinator's severity judgment in the handoff/integration evidence.
