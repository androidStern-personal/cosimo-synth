...
- Build and test entrypoints resolve Cmajor, CHOC, and JUCE through `scripts/resolve_build_dependencies.py`. The resolver uses the shared user-level CPM source cache automatically; do not link dependency trees or prepare a worktree-specific checkout.
...
- The canonical compiled desktop synth build command is `npm run synth:desktop:build`.
- The canonical desktop synth HMR launcher command is `npm run synth:desktop:dev`.
- If a task changes the desktop UI, the final delivery must include a running standalone dev app unless the user explicitly says not to launch it. Run `npm run synth:desktop:dev` after the changes, confirm it starts `http://127.0.0.1:5174`, and confirm it launches `build/desktop_native/CosimoDesktopNative_artefacts/Release/Standalone/CosimoDesktopNative.app`.
- During desktop UI development, do not claim the standalone app is verified or ready for review unless `npm run synth:desktop:dev` started a fresh Vite dev server for this repo, the wrapper was rebuilt against that dev server, and the standalone app was relaunched from that dev session.
...
- For any completed desktop synth feature, final delivery must include building and installing the current VST3 for Ableton unless the user explicitly says not to. Run `npm run synth:desktop:build`, verify it installs `~/Library/Audio/Plug-Ins/VST3/CosimoDesktopNative.vst3`, and report that exact installed path. Do not treat tests plus generated UI bundles as delivered without this Ableton install step.
- `./scripts/build_desktop_native.sh` defaults to the compiled desktop UI. `npm run synth:desktop:dev` now starts the desktop Vite server itself, rebuilds the wrapper in `dev-server` mode, and launches the standalone app against `http://127.0.0.1:5174`.
...
- `scripts/resolve_build_dependencies.py` is the sole Cmajor/CHOC/JUCE source resolver for effect production builds, the desktop native wrapper, iOS AUv3, web/codegen builds, and the repo-built generic `CmajPlugin.vst3`. `cmake/dependencies.lock.cmake` alone owns the immutable repositories and commits. CPM retrieves the private patched Cmajor tree, its pinned private CHOC submodule, and pinned JUCE into a shared user cache; callers receive validated, clean, read-only source paths and keep build output inside their own worktree.
