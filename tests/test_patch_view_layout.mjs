...
test("UI builds emit canonical reproducible desktop and iPhone source maps", async () => {
    const generatedFiles = [
        "patch_gui/desktop/app.js",
        "patch_gui/desktop/app.js.map",
        "patch_gui/index.ios.js",
        "patch_gui/index.ios.js.map",
    ];
    const mapContracts = [
        {
    ...
    for (const { relativePath, dependencyPrefix } of mapContracts) {
        ...
        for (const source of dependencySources) {
            assert.equal(
                ...
                true,
                `${relativePath} dependency source must be repository-relative: ${source}`,
            );
            assert.doesNotMatch(
                source,
                /(?:\.codex\/worktrees|src\/cosimo-synth\/node_modules|^\/Users\/)/,
                `${relativePath} must not encode a checkout-specific dependency path.`,
            );
        }
    }

    const firstBuild = await Promise.all(generatedFiles.map(async (relativePath) => ({
        relativePath,
        contents: await fs.readFile(path.join(repoRoot, relativePath), "utf8"),
    })));
    await runUiBuild();
    for (const generated of firstBuild) {
        assert.equal(
            await fs.readFile(path.join(repoRoot, generated.relativePath), "utf8"),
            generated.contents,
            `${generated.relativePath} must be byte-reproducible from unchanged source.`,
        );
    }
});
...
test("Bounce and Global Tune are append-only additions to the frozen synth host-parameter order", async () => {
    ...
    assert.notEqual(sourceMode, undefined);
    assert.equal(sourceMode.type, "float32");
    assert.match(sourceMode.annotation, /name:\s*"Source Mode"/);
    assert.match(sourceMode.annotation, /min:\s*0(?:\.0)?f?/);
    assert.match(sourceMode.annotation, /max:\s*1(?:\.0)?f?/);
    assert.match(sourceMode.annotation, /init:\s*0(?:\.0)?f?/);
    assert.match(sourceMode.annotation, /discrete:\s*true/);
    assert.match(sourceMode.annotation, /text:\s*"Oscillator\|Bounce"/);
    assert.match(sourceMode.annotation, /rampFrames:\s*0/);
    assert.notEqual(globalTune, undefined);
    assert.equal(globalTune.type, "float32");
    assert.match(globalTune.annotation, /name:\s*"Global Tune"/);
    assert.match(globalTune.annotation, /min:\s*-24(?:\.0)?f?/);
    assert.match(globalTune.annotation, /max:\s*24(?:\.0)?f?/);
    assert.match(globalTune.annotation, /init:\s*0(?:\.0)?f?/);
...
test("desktop standalone loader is emitted from source and stays host-configurable after repeated builds", async () => {
    ...
    assert.match(generatedApp, /\bas createDesktopPatchView\b/);
    assert.match(generatedApp, /\bas default\b/);
    assert.doesNotMatch(
        generatedApp,
        /Generated from ui\/desktop\/standalone-loader\.js by node ui\/build\.mjs\. Do not edit this file directly\./,
    );
...
test("desktop and iPhone wavetable views share the same renderer implementation", async () => {
    ...
    const generatedRenderer = await fs.readFile(path.join(repoRoot, "patch_gui", "wavetable-display.js"), "utf8");

    assert.match(sharedRenderer, /export class CanvasWavetableDisplay/);
    assert.doesNotMatch(sharedRenderer, /from "\.\.\/\.\.\/patch_gui\//);
    assert.match(
        generatedRenderer,
        /Generated from ui\/shared\/wavetable-display\.ts by node ui\/build\.mjs\. Do not edit this file directly\./,
    );
});
...
test("the worker runtime is produced as a real Vite build output instead of a Python-generated source copy", async () => {
    ...
    assert.match(builtWorker, /\bas WavetableWorkerController\b/);
    assert.match(builtWorker, /\bas createWavetableWorkerController\b/);
