...
test("the desktop VST3 qualification probe freezes all T78 automation identities", async () => {
    ...
    assert.match(probeSource, /minimumNormalized=/);
    assert.match(probeSource, /minimumText=/);
    assert.match(probeSource, /maximumNormalized=/);
    assert.match(probeSource, /maximumText=/);
    assert.match(probeSource, /defaultNormalized=/);
    assert.match(probeSource, /defaultText=/);
    assert.match(probeSource, /currentNormalized=/);
    assert.match(probeSource, /currentText=/);
    assert.match(probeSource, /parseNumericText/);
    assert.match(probeSource, /defaultTextToleranceDb/);
    assert.match(probeSource, /reconstructedDefaultDb/);
    assert.match(probeSource, /std::fabs \(reconstructedDefaultDb\) > defaultTextToleranceDb/);
    assert.match(probeSource, /std::fabs \(defaultTextDb\) > defaultTextToleranceDb/);
    assert.doesNotMatch(probeSource, /defaultTextDb - reconstructedDefaultDb/);
    assert.doesNotMatch(probeSource, /getText \(0\.0f, 64\)\.trim\(\) != "-100"/);
...
test("the out-of-process AUv3 smoke freezes and writes every T78 trim identity", async () => {
    ...
    assert.match(harnessSource, /CosimoParseT78NumericText/);
    assert.match(harnessSource, /errno != ERANGE/);
    assert.match(harnessSource, /\*end == '\\0'/);
    assert.match(harnessSource, /std::isfinite \(result\)/);
    assert.match(harnessSource, /CosimoT78DefaultTextToleranceDb/);
    assert.match(harnessSource, /reconstructedDefaultDb/);
    assert.match(harnessSource, /std::fabs\(reconstructedDefaultDb\) > CosimoT78DefaultTextToleranceDb/);
    assert.match(harnessSource, /std::fabs\(defaultTextDb\) > CosimoT78DefaultTextToleranceDb/);
    assert.match(harnessSource, /minimumNormalized=/);
    assert.match(harnessSource, /minimumText=/);
    assert.match(harnessSource, /maximumNormalized=/);
    assert.match(harnessSource, /maximumText=/);
    assert.match(harnessSource, /defaultNormalized=/);
    assert.match(harnessSource, /defaultText=/);
    assert.doesNotMatch(harnessSource, /minimumText isEqualToString/);
    assert.doesNotMatch(harnessSource, /defaultText isEqualToString/);
    assert.doesNotMatch(harnessSource, /maximumText isEqualToString/);
