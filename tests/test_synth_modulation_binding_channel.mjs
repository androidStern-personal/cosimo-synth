...
test("the actual state channel permits an amount-only modulation delta and processes its final host effect", async () => {
    const [{ createMockPluginStateHost }, { createSynthModulationBinding },
        ...
        loadUIModule(root, "ui/worker/synth-modulation-binding.ts"),
    ...
    const listeners = new Map(), events = [], hostEffects = [], writes = [], defects = [], protocolFailures = [];
    ...
    // service, private binding and actual Cmajor channel enforce declarations,
    ...
    const host = createMockPluginStateHost({
        ...
        engine: {
            bindings: [createSynthModulationBinding(runtimeConnection)],
    ...
    try {
        await host.ready;
        await until(() => field()?.application?.kind === "sent");
        assert.deepEqual(field().value, bank);
        assert.equal(events.filter(event => event.endpoint === "modulationMsegBuffer").length, 6);
