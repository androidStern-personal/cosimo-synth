...
Production status: complete through measured fixed-48-kHz history reads, rolling
two-voice boundaries and retriggers, cold-start and authoritative-reset proof,
sparse-v7 persistence, source and packaged UI, official Fontaudio identity, and
four-chain bounds. SeqFX selects zero-added-host-latency rolling lookback,
proportional crossfade, block-relative decay, no tail, and a truthful four-second
high-quality maximum; the evidence and rejected alternatives are recorded in
`research/seqfx/reverse-decision.md`. Listening comparison, spoken-count timing,
and Ableton acceptance remain Phase 8 gates.
...
| Tape is a trigger-latched motor gesture with explicit Stop Time, Curve, Crossfade-to-Live/Spin-Up return, optional Start Time, and Sync/Free timing | Committed and implemented | Official mature-product vocabulary plus the frozen benchmark; block end does not define motor lifetime; Tape Start means 0x to 1x, while temporal catch-up is a separate documented behavior | Pretend a <=1x read head can catch a moving live head, invent Tail/Throw controls, or keep the old block-bound ramp | Two gesture voices, tiered history, explicit v5 migration, bounded 10 ms handoffs, and a dedicated duration-truthful v2 inspector |
